"""
Review Trigger Agent 実行スクリプト（v2.5.0）

ReviewTriggerAgent（「AiPublishReviewService（公開前レビューレポート生成）を今実行すべきか」を
...
注意:
    ...
    - manager.run() は AgentManager に登録されているすべての Agent
      ...
      同じタスクで実行する。Agentを個別に選んで実行する仕組みは
      現時点では存在しない（run_publish_trigger_agent.py と同じ制約）。
...
def main():
    parser = argparse.ArgumentParser(
        description="Review Trigger Agent 実行スクリプト（v2.5.0）"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        default=False,
        help="実際のレビューレポート生成は行わず、判断結果のみ確認する（Agent側dry_run）",
    )
    parser.add_argument(
        "--article-id",
        default=None,
        metavar="SLUG",
        help="レビュー実行時に絞り込む記事ID（未指定: 全件）",
    )
    ...
    print("=" * 50)
    print("Review Trigger Agent 開始")
    print("=" * 50)
    if args.dry_run:
        print("  モード: DRY RUN（Agent側。実際のレビューレポート生成は行いません）")
    if args.article_id:
    ...
    print()
    print("=" * 50)
    print(f"Review Trigger Agent 完了: {len(results)} 件のAgentを実行しました")
    print("=" * 50)
    ...
    if not any(r.agent_name == "review_trigger_agent" for r in results):
        print("[情報] ReviewTriggerAgent は実行されませんでした。")
        print("  REVIEW_TRIGGER_AGENT_ENABLED=true を .env に設定してください（二重ゲート）。")
