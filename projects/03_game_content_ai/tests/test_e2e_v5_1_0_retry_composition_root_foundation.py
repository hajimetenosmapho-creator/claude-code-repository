...
# 24章）。
_ALLOWED_WORKFLOW_ENGINE_CHANGES_19 = {
...
if git_available:
    ...
    _prefix_proc_19 = subprocess.run(
        ["git", "rev-parse", "--show-prefix"],
        cwd=str(PROJECT_ROOT), capture_output=True, text=True, timeout=10,
    )
    _repo_prefix_19 = _prefix_proc_19.stdout.strip()
    for _label, _dir, _allowed in (
        ...
        for line in _dir_diff.stdout.splitlines():
            if not line.strip():
                continue
            _p = line[3:].strip()
            if _repo_prefix_19 and _p.startswith(_repo_prefix_19):
                _p = _p[len(_repo_prefix_19):]
            _dir_changed.add(_p)
        check_true(
            f"19. {_dir}の差分はRelease 6.30承認済み変更のみに限定される",
