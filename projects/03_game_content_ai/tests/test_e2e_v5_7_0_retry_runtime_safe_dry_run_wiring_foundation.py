...
# 24章）。
_ALLOWED_WORKFLOW_ENGINE_CHANGES_24 = {
...
_ALLOWED_RETRY_COMPOSITION_CHANGES_24 = {
...
_ALLOWED_RETRY_ENGINE_CHANGES_24 = {
...
_ALLOWED_RETRY_RUNTIME_ORCHESTRATOR_CHANGES_24 = {
...
if git_available:
    ...
    _prefix_proc_24 = subprocess.run(
        ["git", "rev-parse", "--show-prefix"],
        cwd=str(PROJECT_ROOT), capture_output=True, text=True, timeout=10,
    )
    _repo_prefix_24 = _prefix_proc_24.stdout.strip()
    for _label, _dir, _allowed in (
        ...
        ("retry_composition", "src/retry_composition", _ALLOWED_RETRY_COMPOSITION_CHANGES_24),
        ("ai", "src/ai", _ALLOWED_AI_CHANGES_24),
        ("retry_engine", "src/retry_engine", _ALLOWED_RETRY_ENGINE_CHANGES_24),
        (
        ...
        for line in _dir_diff.stdout.splitlines():
            if not line.strip():
                continue
            _p = line[3:].strip()
            if _repo_prefix_24 and _p.startswith(_repo_prefix_24):
                _p = _p[len(_repo_prefix_24):]
            _dir_changed.add(_p)
        check_true(
            f"24. {_dir}の差分はRelease 6.30承認済み変更のみに限定される",
