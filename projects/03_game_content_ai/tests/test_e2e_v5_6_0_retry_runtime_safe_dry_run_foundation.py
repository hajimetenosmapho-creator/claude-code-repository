...
# 24章）。
_ALLOWED_WORKFLOW_ENGINE_CHANGES_23 = {
...
_ALLOWED_EXECUTION_HISTORY_CHANGES_23 = {
...
if git_available:
    ...
    _prefix_proc_23 = subprocess.run(
        ["git", "rev-parse", "--show-prefix"],
        cwd=str(PROJECT_ROOT), capture_output=True, text=True, timeout=10,
    )
    _repo_prefix_23 = _prefix_proc_23.stdout.strip()
    for _label, _dir, _allowed in (
        ("workflow_engine", "src/workflow_engine", _ALLOWED_WORKFLOW_ENGINE_CHANGES_23),
        ("execution_history", "src/execution_history", _ALLOWED_EXECUTION_HISTORY_CHANGES_23),
    ):
        ...
        for line in _dir_diff.stdout.splitlines():
            if not line.strip():
                continue
            _p = line[3:].strip()
            if _repo_prefix_23 and _p.startswith(_repo_prefix_23):
                _p = _p[len(_repo_prefix_23):]
            _dir_changed.add(_p)
        check_true(
            f"23. {_dir}の差分はRelease 6.30承認済み変更のみに限定される",
