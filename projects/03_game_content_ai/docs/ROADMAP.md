...
- [ ] **`scripts/run_retry_runtime.py`の`--loop` / `--daemon`対応**：本Releaseの実運用結果を踏まえて必要性を再判断する。Windows タスクスケジューラ等の外部プロセス管理に委ねる現方針の見直しを含む（v5.4.0 Future Architecture Consideration）
