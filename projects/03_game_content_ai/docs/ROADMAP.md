...
- [ ] **Retry Runtime Safe Dry Run Wiring**：`scripts/run_retry_runtime.py`に`argparse`・`--dry-run`フラグを新規導入し、`RetryRuntimeOrchestrator.run_once(dry_run=...)`（v5.6.0）をCLIから安全に試せるようにする。v5.6.0のArchitecture Review（ChatGPTレビュー反映）で、Execution Release（`run_once()`本体の変更）とEntry Point Release（CLI配線）を分離するという本プロジェクトの一貫したパターン（v5.3.0/v5.4.0と同型）を踏襲する形で次Release候補として切り出された
- [ ] **Retry Enqueue Trigger Dry Run Foundation**：`RetryEnqueueTrigger.enqueue_pending_failures()`に`dry_run`引数を追加し、Retry Queueへの新規登録（enqueue）自体を抑止できるようにする。v5.6.0時点では`run_once(dry_run=True)`を呼んでも、WorkflowMonitor上のFAILED/TIMEOUTをRetry Queueへenqueueする処理自体は通常どおり実行される（Queueへの追加はin-memoryで可逆的・外部作用を伴わないためExecution/Removal/Historyよりリスクレベルが異なると判断し対象外とした。ユーザーから見ると「dry_runと言ったのに何かは変わる」という違和感につながりうるため、Known Issueに留めず次Release候補として明示した）
...
- [ ] **`scripts/run_retry_runtime.py`への`RetryRuntimeLoop`配線（`--loop`対応）**：本Releaseでは未着手。dry_run安全性（Safe Dry Run Foundation）はv5.6.0で解消したため、Retry Runtime Safe Dry Run Wiring（`--dry-run`のCLI配線）完了後に着手要否を改めて判断する。Windows タスクスケジューラ等の外部プロセス管理に委ねる現方針の見直しを含む（v5.4.0・v5.5.0・v5.6.0 Future Architecture Consideration）
