...
- [ ] **安全なdry_runの再設計**：`RetryQueueUpdateDecider`等のDecider/Executor層が`dry_run`を認識できるようになった段階で、`run_once(dry_run=...)`を独立Releaseとして再設計し、`scripts/run_retry_runtime.py`側にも`--dry-run`を追加する（v5.3.0 Known Issue、v5.4.0 Future Architecture Consideration）
...
- [ ] **`scripts/run_retry_runtime.py`への`RetryRuntimeLoop`配線（`--loop`対応）**：本Releaseでは未着手。dry_run安全性（Safe Dry Run Foundation）の状況を踏まえて着手要否を改めて判断する。Windows タスクスケジューラ等の外部プロセス管理に委ねる現方針の見直しを含む（v5.4.0・v5.5.0 Future Architecture Consideration）
