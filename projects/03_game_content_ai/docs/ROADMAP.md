...
- [ ] **Retry Enqueue Guard（無限再投入対策の完成）**：v4.7.0で記録した`RetryHistoryManager`の再試行履歴を`RetryEnqueueTrigger.enqueue_pending_failures()`側が参照し、既に再試行済みの`run_id`を再enqueueしないようにする新規コンポーネント。v4.6.0 Known Issue・v4.7.0 Future Extensionの完成形（`docs/design/retry_history_foundation.md` 10章）
