...
- [ ] **`SKIPPED`（`max_attempts`到達）のQueue滞留対応**：`NOOP`と判定された項目（特に`SKIPPED`＝`max_attempts`到達）は、v4.2.0でも除去する手段を持たず恒久的にQueueへ滞留し得る。除去する（`FAIL`相当として扱う）・Dead Letter Queueへ振り分ける等の対応は、ユーザー指示によりv4.2.0では意図的にスコープ外とされ、次Release以降の検討事項として持ち越された（`docs/design/retry_queue_update_foundation.md` 12章 Future Extension・16.3節 Recommendation 2、`docs/design/retry_queue_removal_foundation.md` 12章 Future Extension）
