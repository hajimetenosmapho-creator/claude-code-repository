...
- [ ] **Retry Monitoring Foundation**（次候補）：v6.3.0の`RetryMetricsSnapshot`を入力として受け取り、閾値（例：`enqueue_success_ratio`低下等）に基づいて健全性ステータス（`HEALTHY` / `DEGRADED` / `UNHEALTHY`等）を判定するだけの別パッケージ（`src/retry_monitoring/`）。`Metrics → Monitoring → Alert`の一方向依存のみを許可し、`Monitoring → Metrics`への逆依存は禁止する。通知（Slack／メール等の外部I/O）は責務が異なる別の後続Releaseへ切り出す（`docs/design/retry_metrics_foundation.md` 11.1節）
