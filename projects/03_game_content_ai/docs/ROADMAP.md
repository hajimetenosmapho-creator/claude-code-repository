...
- [ ] **Retry Alert Foundation**（次候補）：v6.4.0の`RetryHealthReport`を入力として受け取り、`status`が`DEGRADED`／`UNHEALTHY`の場合にSlack／メール等の外部サービスへ通知する別パッケージ（`src/retry_alert/`）。`Alert → Monitoring`への逆依存は禁止する。外部I/O（Slack API等）を伴うため独立したArchitecture Reviewを要する（`docs/design/retry_monitoring_foundation.md` 11.1節）
