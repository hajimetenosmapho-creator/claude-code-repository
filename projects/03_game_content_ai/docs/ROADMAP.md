...
- [ ] **Retry Enqueue TriggerのComposition Root**：`RetryEnqueueTrigger`を定期的に呼び出し、その結果を`RetrySchedulerSource`〜`RetryManager`の下流パイプラインへつなげる実運用の起動スクリプト（v4.6.0では新設Adapterの実装のみに留め、定期実行の仕組みは対象外とした）。v5.0.0でGuardの判定基準精緻化が完了したことで、`RetryPolicy.from_env().max_attempts`を`enqueue_pending_failures(max_attempts=...)`へ実際に注入する前提条件が整った
