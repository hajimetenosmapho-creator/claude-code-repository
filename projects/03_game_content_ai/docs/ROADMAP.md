...
- [ ] Retry History（再試行回数の永続化）・RetryDecision（Retry可否判定の専用コンポーネント化）・RetryReason Enum・Exponential Backoff・Adaptive Retry・Failure Classification・AI Retry Decision・Parallel/Distributed Retry・Circuit Breaker・Manual Retry UI・Notification（Retry Engine v3.0.0ではいずれも対象外。詳細は`docs/design/retry_engine_foundation.md` 11章 Future Extensions）。**Exponential Backoff / Adaptive Retryについては、v4.5.0で`RetryDecisionPolicy` / `ExplainableRetryPolicy`（Protocol）という差し替え可能な構造が整備済みであり、実装時はこれらの契約を満たす新クラスを追加するだけでよい（`RetryManager`側の追加改修は不要。`docs/design/retry_policy_foundation.md`）** **Retry History（再試行回数の永続化）については、v4.6.0の`RetryEnqueueTrigger`が抱える無限再投入リスク（Queueから除去された`run_id`がWorkflow Monitor上でなおFAILED/TIMEOUTのまま観測され続ける問題）の対策候補としても位置づけられる。`RetryExecutor`が再実行のたびに新しい`run_id`を発行し`metadata["retried_from"]`に元`run_id`を記録している（v3.0.0、無改修）ことを手掛かりにできる（`docs/design/retry_enqueue_trigger_foundation.md` 11章）**
- [ ] **Retry Enqueue Triggerの無限再投入対策**：v4.6.0の`RetryEnqueueTrigger`は`RetryQueueManager.exists()`によるQueue内重複防止のみを行い、Queueから除去（`COMPLETE` / `FAIL` / `CLEANUP`）された後もWorkflow Monitor上でなお`FAILED` / `TIMEOUT`のまま観測され続ける`run_id`を再度enqueueしてしまう可能性がある（`docs/design/retry_enqueue_trigger_foundation.md` 11章 Known Issue）。本Releaseでは呼び出し元（Composition Root）が存在しないため実害はないが、Composition Root実装前に対策が必要
- [ ] **Retry Enqueue TriggerのComposition Root**：`RetryEnqueueTrigger`を定期的に呼び出し、その結果を`RetrySchedulerSource`〜`RetryManager`の下流パイプラインへつなげる実運用の起動スクリプト（v4.6.0では新設Adapterの実装のみに留め、定期実行の仕組みは対象外とした）
