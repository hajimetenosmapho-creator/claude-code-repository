...
| [`../development_workflow.md`](../development_workflow.md) | Development Workflow v1.0。Releaseの分類（Architecture Release／Fast Track／Emergency Fix）と、分類ごとに必要な成果物・レビュー量・ドキュメント量（＝既存Architecture Processをどの重さで適用するか）を定める上位ルール。個々の手続きの中身（Git運用・レビュー観点・責務分離等）は上記4文書側の記載を正とし、本文書では重複して定義しない | 低 |
| [`handoff_template.md`](./handoff_template.md) | チャット引継ぎ用テンプレート。セッション固有の7項目のみを埋めて新しいチャットに貼り付ける | 更新不要（テンプレート） |

各文書はそれぞれの役割にのみ内容を閉じ、他文書がカバーする内容は繰り返さず参照する形を取っています（例：`ai_collaboration_guide.md`のGit運用・モデル運用は`project_instructions.md`側の該当章を参照するのみ）。`development_workflow.md`は本ディレクトリの外（`docs/`直下）に配置されていますが、既存Architecture Processに対する上位ルールという役割上、本表に含めています。
...
1・2が根本規範（What／Why）、3がそこから派生する協業・進行手順（Who／When）、4がリリース単位の個別設計、5・6がセッション単位の情報という位置づけです。Development Workflow（`../development_workflow.md`）の位置付けの詳細は`project_instructions.md` 13章を参照してください。
