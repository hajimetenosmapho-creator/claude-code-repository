...
作成日：2026-06-26  
更新日：2026-09-13（v6.33.0 — Retry Observability Runtime Integrationを追記。`RetryObservabilityPipeline`（v6.29.0）をRetry Runtime本体（`scripts/run_retry_runtime.py`）へ実配線し、`scripts/show_retry_notification.py::build_report()`が保持し続けていた合成ロジックの重複を解消したこと。新規独立package`src/retry_runtime_observability/`（`RetryRuntimeObservabilityReporter`）を新設し、`run_cycle()`内で`cycle_logger.log_cycle()`成功時のみ呼び出すCurrent-Cycle Inclusion Contractを確立したこと、Runtime・CLIともfull-history semanticsを維持しwindowingを導入しなかったこと、`RetryRuntimeCycleLogger.log_cycle()`の戻り値を`None`→`bool`（fsync durability等は保証しない限定的な契約）へ追加的に変更したこと、読み取り・評価・整形・コンソール出力の全段階を覆う単一failure containment境界（`Exception`のみcontain、`BaseException`は対象外）を確立したこと、Architecture Design（Codex独立read-only review 4ラウンド、Round 4で`APPROVED`：Blocking/Major/Minor/Suggestions いずれも0）・新規E2E（v6.33 E2E PASS。real `.run/retry_runtime.lock`有無でアサーション総数が186件・187件のいずれかになる条件分岐を含む）・既存v6.8.0 CLI Wiring E2E（197/197 PASS）を経たこと、retry判断・durable state・side-effect safetyはいずれも変更していないこと、external sender・Scheduler統合・Bounded Tail Window（windowing）はいずれもOut of Scope／Future Candidateとしたことを明記。詳細は本ファイル内「Retry Observability Runtime Integration層」節を参照）
...
## Retry Monitoring Foundation層（`src/retry_monitoring/`、v6.4.0 追加）

v6.3.0が生成する`RetryMetricsSnapshot`**のみ**を入力として受け取り、あらかじめ定義した閾値
（`degraded_below=0.8` / `unhealthy_below=0.5`）に基づいて健全性ステータス（`RetryHealthStatus`：
`HEALTHY` / `DEGRADED` / `UNHEALTHY`）を判定するだけの新規独立パッケージ`src/retry_monitoring/`
（`RetryHealthStatus` / `RetryHealthThresholds` / `RetryHealthReport` / `RetryHealthEvaluator`）を
追加した。Judgment Only Foundationであり、Runtime・Logger・JSONL・Metricsのいずれへも一切
フィードバックを行わない。

```
RetryMetricsSnapshot（v6.3.0が生成した集計結果。本Foundationの唯一の入力）
        │
        ▼
...
### 依存方向（Metrics → Monitoring → Alert の一方向のみ）

`retry_monitoring`がimportするのは`retry_metrics`（`RetryMetricsSnapshot`型の参照のみ）と標準
ライブラリ（`enum` / `dataclasses`）のみであり、`retry_runtime_logging` /
...
（`retry_metrics → retry_monitoring`）も発生しない。この契約は新規E2Eテストのソースコード走査
（import文の検出・`open(`/`pathlib.Path`/`.jsonl`という文字列の不在確認）により機械的に保証する。

### RetryHealthThresholds・RetryHealthReportの設計判断

- `RetryHealthThresholds`は**Immutable Value Object**（`frozen=True`のdataclass）であり、
  Config（外部設定・環境変数）ではなくMonitoring DomainのDomain Valueとして位置づける。
  Foundationの責務は固定値を保持することのみであり、値の外部化は対象外（将来検討）
- `RetryHealthEvaluator`はThresholdを自ら生成しない。Constructor Injectionで外部から受け取るか、
  未指定時は`RetryHealthThresholds()`のDefault Thresholdを使用するのみを許可する
- `RetryHealthReport`はRelease 6.4では`status`のみを保持するImmutable（frozen dataclass）な値
  オブジェクトとした。`reason` / `warnings` / `details`（または`violations`）等の診断情報は
  将来拡張の対象とし、本Releaseでは実装しない
- `RetryHealthEvaluator`はStateless Pure Functionであり、同一の`RetryMetricsSnapshot`に対し常に
  同一の`RetryHealthReport`を返す

### Runtime Pipeline・retry_metricsへ一切手を加えない設計判断
...
（`RetryMetricsCalculator` / `RetryMetricsSnapshot` / `RetryRuntimeLogReader` /
`RetryRuntimeLogRecord`）はいずれも無改修。本Foundationからのフィードバック（Runtime停止・
Runtime制御・Retry Queue更新・Scheduler通知等）は一切発生しない。

### Future Extension

Retry Alert Foundation（`RetryHealthReport`の`status`が`DEGRADED`／`UNHEALTHY`の場合にSlack／
メール等へ通知する別パッケージ）・Retry Monitoring CLI/Report Wiring Foundation・閾値の外部設定化・
複数指標に基づく総合判定・`RetryHealthReport`への`reason`/`warnings`/`details`追加（詳細は
`docs/ROADMAP.md`）。
...
## Retry Notification Message Foundation層（`src/retry_notification_message/`、v6.7.0 実装完了）

> **本節は実装完了時点の記録である。2026-07-15時点で新規E2E（21シナリオ・117アサーション・117/117 PASS）・既存Regression（v5.9.0〜v6.6.0、826/826 PASS、合計943/943 PASS）とも完全PASSし、Test Review「Approved」・Code Review「Approved」を経ている。ただしRuntime Wiring・CLI Wiringはいずれも未実装（本Releaseの対象外）であり、他4パッケージと同様「消費者不在の先行実装」のままである。**

v6.6.0が生成する`RetryNotificationDecision`**のみ**を入力として受け取り、固定の通知Message Value Object（`RetryNotificationMessage`）を構築するだけの新規独立パッケージ`src/retry_notification_message/`（`RetryNotificationMessage` / `RetryNotificationMessageBuilder`）を追加した。判定（Judgment）ではなくValue Buildingを担当し、Runtime・Logger・JSONL・Channel・Senderのいずれへも一切フィードバックを行わない。

```
RetryNotificationDecision（v6.6.0が生成した判定結果。本Foundationの唯一の入力）
        │
        ▼
RetryNotificationMessageBuilder.build(decision) -> RetryNotificationMessage
        │
        ▼
（呼び出し元は本Releaseでは未定。消費者不在の先行実装。将来はChannel/Sender Foundationが消費）
```

### データフローとimport依存方向（区別して記述）

**データフロー（実行順序）**：`Metrics → Monitoring → Alert → Notification → Notification Message`。Message FoundationはNotification Decisionの直接の後続Layerである。
...
`retry_notification_message`がimportするのは`retry_notification`（`RetryNotificationDecision` / `RetryNotificationStatus`型の参照のみ）と標準ライブラリのみであり、`retry_alert` / `retry_monitoring` / `retry_metrics` / Runtime系 / `RetryCompositionRoot` / Logger / `scripts/`（CLI） / 外部ライブラリのいずれもimportしない。逆方向（`retry_notification → retry_notification_message`）も発生しない。この契約は新規E2Eテストのソースコード走査（AST解析、相対importの`level>=2`検出を含む）により機械的に保証されており、Code Reviewで`__all__`の集合検査のみでは名前空間全体の想定外露出まで保証できない点が指摘され、`hasattr()`による直接検査を追加した。

### Architecture Reviewの経緯（入力設計・Dependency方向の修正）

初版のArchitecture Designでは`build(alert: RetryAlert) -> RetryNotificationMessage`という入力設計・`retry_notification_message → retry_alert`という直接依存を提示したが、ChatGPT Architecture Review（1回目「Changes Required」）で「Message層がNotification Decisionを迂回する」「AlertからNotificationとMessageが分岐する構造になる」「直前Layerだけへ依存する既存Architectureの一貫性を崩す」との指摘を受けた。修正の結果、入力を`build(decision: RetryNotificationDecision) -> RetryNotificationMessage`へ、Dependencyを`retry_notification_message → retry_notification`のみへ変更し、2回目のArchitecture Reviewで「Approved」となった。

### 変換規則（Design Contract）とMessageの意味契約

- `RetryNotificationMessageBuilder`は閾値判定・通知要否判定を一切行わない。`RetryNotificationEvaluator`（v6.6.0）が既に確定した`status`を、以下の固定対応表に従って`RetryNotificationMessage`へ変換するだけの単純な写像である。`NOTIFY → RetryNotificationMessage(body=<固定通知文言>)`
- `RetryNotificationStatus.NO_NOTIFICATION`をBuilderへ渡すことは契約違反として扱い、`ValueError`を送出する（Fail Fast）。「評価の正常結果として通知対象ではない」ことと「Messageを生成できる」ことは異なる概念であり、混同しない
- 既知の2 Status（`NO_NOTIFICATION` / `NOTIFY`）以外が渡された場合も同様に`ValueError`を送出する
- `RetryAlertLevel.WARNING`／`CRITICAL`はいずれも`NOTIFY`へ収束するため、Builderは両者を区別できず区別しない。共通の固定Messageを生成する。Message本文は「通知対象状態の存在を伝える」ことのみを目的とし、重大度・原因・時刻・対象・対応手順は表現しない

### RetryNotificationMessageの設計判断

- `RetryNotificationMessage`はRelease 6.7では`body`のみを保持するImmutable（frozen dataclass）な値オブジェクトとした。初版では`level: RetryAlertLevel`の保持を検討したが、Architecture Reviewで「将来Channel要件の先読みになる」「v6.6.0で意図的に残した重大度非保持の責務境界を実質的に変更する」との指摘を受け、`body`のみへ縮小した。重大度が必要な将来の消費者は、v6.6.0のTechnical Debt方針どおり、呼び出し元が保持する元の`RetryAlert`を別途参照する
- `RetryNotificationMessageBuilder`はStateless Pure Functionであり、同一の`RetryNotificationDecision`に対し常に同一の`RetryNotificationMessage`（`NOTIFY`の場合）または同一の例外（`NO_NOTIFICATION`・未対応値の場合）を返す

### Runtime Pipeline・既存4パッケージへ一切手を加えない設計判断

`RetryRuntimeLock` / `RetryRuntimeShutdown` / `RetryRuntimeLoop` / `RetryRuntimeOrchestrator` / `RetryManager` / `RetryCompositionRoot` / `RetryRuntimeCycleLogger`、`src/retry_metrics/`、`src/retry_monitoring/`、`src/retry_alert/`、`src/retry_notification/`はいずれも無改修。本Foundationからのフィードバックは一切発生しない。

### Test Review・Code Review・Regressionの実績

新規E2E（`tests/test_e2e_v6_7_0_retry_notification_message_foundation.py`）は21シナリオ・117アサーション・117/117 PASS（終了コード0、警告なし）。既存Regression Suite（v5.9.0〜v6.6.0、826/826 PASS）とあわせた総確認数は943/943 PASS。Test Review・Code Reviewともに「Approved」。Code Reviewで発見した唯一の指摘（Public API検査が`__all__`の集合一致のみでは名前空間全体の想定外露出を保証できない点）は、E2Eテストへの`hasattr()`検査追加（production code無改修）で解消した。

### 責務境界（Out of Scope）

`RetryAlert`入力・`RetryAlertLevel`のMessage保持・WARNING／CRITICAL別Message・チャネル選択・実送信（Slack／メール／Webhook等）・Suppression／重複排除／レート制限・Recovery通知・履歴の永続化・Runtime Wiring・Composition Root Wiring・CLI表示は、いずれも本Releaseの対象外である。

### Future Extension

Retry Notification Channel Foundation・Retry Notification Delivery／Sender Foundation・Retry Alert／Notification Message CLI Report Wiring Foundation・Severity-aware Message Foundation・Message Template Foundation・Localization・Runtime／Scheduler Integration・Suppression／Deduplication／Rate Limiting（順序は確定事項として固定しない。詳細は`docs/ROADMAP.md`）。
...
> **v6.23.0 追記**：本層が確立した`WordPressMediaUploadErrorReason`（12値）は
> v6.23.0でも**一切変更していない**（v6.23.0のE2E `COMPAT-WP-REASON-12`／
> `COMPAT-WP-ALL`で12値・`__all__` 4 symbolの不変を確認済み）。
> 上記Deferredのうち**S2R-1（DEF-6.22-15）はv6.23.0で完了**した
> （`tests/test_e2e_v6_22_0_*.py`へ説明コメントを追記。Release Reviewが
> `_scan_noparse_violations()`のASTがHEADと完全一致することを確認しており、
> 検査ロジック・`_FORBIDDEN_ATTRS`・assertion件数はいずれも不変）。
> **M5-1は継続**（v6.23.0は構築形guardを必要とせず判断機会が発生しなかった）。
> **DI-11はv6.23.0で前半（`REQUEST_REJECTED`のSDK例外型による4値細分化）が完了**し、
> 後半（`UNKNOWN`の2経路分離・`INVALID_RESPONSE`の細分化）が残る。
> 詳細は次節「OpenAI Image Generation API Rejection Reason Classification Foundation層」。

---

## OpenAI Image Generation API Rejection Reason Classification Foundation層（`src/openai_image_generation/` ＋ `src/image_generation_fallback_policy/`、v6.23.0）

> **本節はRelease Review Finalize完了時点の記録である。Release Reviewは
> Approved with Suggestions（Blocking 0件・Major 0件・Minor 1件：RR-M-1
> （v6.23 E2EのNOIMPACT陽性対照2件が実guard値を参照しない恒真式。guard本体の
> coverage／equality検査は健全でありProduction behavior・安全性・Runtime Action
> Zero Diffへの影響なし。DEF-6.23-12としてDeferred化し本Releaseでは修正しない）・
> Suggestion 2件：RR-S-1／RR-S-2（いずれも記録のみ・修正不要））で完了し、
> Release 6.23として完了した。** 新規E2E（standalone script形式、
> `tests/test_e2e_v6_23_0_openai_image_generation_api_rejection_reason_classification_foundation.py`、
> API-／COMPAT-REJECTED-／CLS-／ORDER-／E2E-／MSG-／CHAIN-／POLICY-／REJECTSET-／
> CONT-／ZERODIFF-／RUNTIME-／NOPARSE-／SEC-／DEP-／NOEXC-／COMPAT-／NOIMPACT-／
> SOCKET-／ENV- の各Scenario prefix、**332アサーション、332/332 PASS**、終了コード0）・
> 累積Formal Regression（正式Inventory**26ファイル**：`test_e2e_v1_11_0_save_result.py`・
> `test_e2e_v5_9_0_*.py`・`test_e2e_v6_0_0_*.py`〜`test_e2e_v6_23_0_*.py`、
> **既存25ファイル3721/3721 PASS＋新規v6.23.0 332/332 PASS＝総合4053/4053 PASS**、
> FAIL 0・SKIP 0・終了コード異常0・外部接続0）とも完全PASSした。
> **既知差分は`test_e2e_v6_19_0_*.py`の254→262（+8。Enum値追加に対する
> `_ALL_REASONS`駆動ループ2箇所の構造的追随）のみ**であり、v6.11.0（248）・
> v6.21.0（147）・v6.22.0（324）はいずれも件数不変、他21ファイルもv6.22.0実績から
> 変動なし。設計時の見込み値（N = 332／3721／4053）はすべて実測と一致し乖離0件。
> Architecture Review（「Approved with Findings」、Blocking 0・Major 0・Minor 4・
> Suggestion 2）→ Test Review 初回（「Approved with Suggestions」、Blocking 0・
> Major 2・Minor 4・Suggestion 2）→ Test Review 再判定（「**Changes Required**」、
> 未解消Major 1：NOPARSE guardがdeny-list方式であり未知属性を検出できない）→
> Test Review Findings反映（**positive allow-list方式への転換**で解消）→
> Production Implementation → 限定テスト（1313/1313 PASS）→ Formal Regression →
> Release Review という経緯を経ている。

### Purpose

v6.11.0 `_classify_api_error()`が単一の`REQUEST_REJECTED`へ集約していた4つのSDK
例外型を、**SDK例外型だけを根拠として**4値へ細分化する（DI-11前半）。
記事固有の失敗と全記事へ反復するsystemic failureを将来区別できるようにするための
**分類手段の確立**が目的であり、**判断（CONTINUE／PROPAGATE）は一切変更しない**。

```
openai SDK 例外
        │
        ▼
_classify_api_error(exc)   ← exc は isinstance(exc, ...) の第1引数としてのみ使用可（I-EXC-1）
        │
        ├─ BadRequestError          (400) → BAD_REQUEST            ★v6.23.0
        ├─ NotFoundError            (404) → RESOURCE_NOT_FOUND     ★v6.23.0
        ├─ ConflictError            (409) → CONFLICT               ★v6.23.0
        ├─ UnprocessableEntityError (422) → UNPROCESSABLE_ENTITY   ★v6.23.0
        └─ （既存6型 ＋ catch-all は不変）
        │
        ▼
OpenAIImageGenerationError(message【凍結】, reason)
        │
        ▼
decide_image_generation_fallback(error)        ← v6.19.0
        │
        ├─ reason ∈ _REJECTED_REASONS（5値）★v6.23.0で新設
        │      → IMAGE_GENERATION_REQUEST_REJECTED / PROPAGATE_ORIGINAL_ERROR
        ├─ reason ∈ _CONTINUABLE_REASONS（4値・不変）
        │      → IMAGE_GENERATION_FAILED / CONTINUE_WITHOUT_FEATURED_MEDIA
        └─ その他 → UNCLASSIFIED / PROPAGATE_ORIGINAL_ERROR
```

### Package Boundary

**新規packageは作成しない。** 既存2packageの各1ファイルのみを変更する。

```
src/openai_image_generation/
    openai_image_generator.py        ← 変更（Enum 4値追加・分類分岐4分割）
    __init__.py                      ← 無変更（新symbolなし。__all__は3 symbolのまま）

src/image_generation_fallback_policy/
    image_generation_fallback_policy.py  ← 変更（_REJECTED_REASONS新設・分岐条件）
    __init__.py                      ← 無変更（__all__は4 symbolのまま）
```

### reason taxonomy（9値 → 13値）

既存9値の**name・value・定義順は1文字も変更していない**。新4値は末尾へ追加する。

| # | name | value | 由来（SDK例外型／HTTP） |
|---|---|---|---|
| 1〜5 | `AUTHENTICATION`／`PERMISSION_DENIED`／`RATE_LIMIT`／`TIMEOUT`／`CONNECTION` | 既存 | 既存（不変） |
| 6 | `REQUEST_REJECTED` | `request_rejected` | **後方互換のため保持**。productionからは生成されない |
| 7〜9 | `SERVER_ERROR`／`INVALID_RESPONSE`／`UNKNOWN` | 既存 | 既存（不変） |
...
`REQUEST_REJECTED`を削除しない理由は、①`__all__`経由で公開されたPublic APIであること、
②既存E2Eが直接構築していること、③production到達不能な公開値は害がないこと、
④将来SDKが新しい4xx型を追加した際の受け皿として再利用できること、である。

### 分類契約（例外型のみ・positive allow-list guard）

分類は`isinstance`のみに基づき、**response body・exception message・HTTPステータス
コード・例外の属性値はいずれも読み取らない**。この契約は
**例外引数使用形のpositive allow-list方式AST guard（I-EXC-1）**で機械的に強制する。
...
【I-EXC-1】_classify_api_error() の関数本体において、例外引数を表す識別子の
          ...
          これ以外のあらゆる出現文脈は違反とする。
          例外引数名はハードコードせず、第1 positional parameter から AST で決定する。
```

**禁止属性を列挙しない**ため、SDKが将来追加する未知属性（`exc.<new_attr>`）も
自動的に禁止側へ落ちる。これはv6.22.0の`GUARD-WMUE-CONSTRUCTION-SHAPE`が
「識別子の出現文脈をallow-listで縛ることで迂回形の個別列挙を不要にした」のと
**同型の設計**である。Test Reviewが**deny-list方式（禁止属性列挙）では未知属性を
検出できない**と指摘（再判定Major）したことを受けて転換した。

guardは陽性対照16形・陰性対照4形を持ち、Release Reviewの独立検証では
**さらに28の追加攻撃形（別名束縛`_is = isinstance; _is(exc, X)`・ネスト関数
クロージャ・`lambda`・引数再束縛・`exc.args[0]`・`type(exc)`・`%`演算子等）を
すべて検出**し、正当形5形はすべて通過した（偽陽性0）。
本番実装は`exc`のName出現**10件・全件allow形・違反0件**である。

検査対象は`_classify_api_error()`の`ast.FunctionDef`本体のみに限定する
（module全体へ適用すると`generate()`の`self`・`_build_generated_image()`の
`response`が偽陽性となることを実測確認済み）。

### fallback policyの全数写像（13値）
...
写像する。category splitは**4/1/2/2 → 4/5/2/2**となる。

`_CONTINUABLE_REASONS`と同じく**allow-list**（deny-listではない）であるため、
v6.11が将来さらにreasonを追加しても新値は自動的に`UNCLASSIFIED`（安全側）へ
落ちるというv6.19.0のC-17性質を破壊しない。

**CONTINUE対象（`_CONTINUABLE_REASONS`の4値）は1値も拡大していない。**
`IMAGE_GENERATION_FAILED`へ写像されるreasonが4件のままであることが直接証拠であり、
v6.19.0の既存assertion `CONT-EXACTLY-4`が**期待値を変更せずPASSし続ける**ことでも
確認される。

### Runtime Action Zero Diff（Z-1〜Z-8）

> **本Releaseが保証する不変範囲はRuntime Action Zero Diff（Z-1〜Z-8）に限定される。**
> public属性`OpenAIImageGenerationError.reason`は対象4型に対して**意図的に別の値に
> なる**ため、Production behavior全体が不変であるとは記載できない。安全性の根拠は
> 「観測可能な挙動が変わらないこと」ではなく、**「reasonを読む消費者がv6.19 policy
> ただ1つであり、その写像を同時に更新したため下流の結論が変わらないこと」**である。

主張するZero Diffは次の8点に限定される。

| ID | 内容 |
|---|---|
| **Z-1** | Runtime Action Zero Diff（13 reasonすべてで`action`・`status`がv6.22.0時点と一致） |
| **Z-2** | Category Zero Diff（`decision.category`が一致。policy更新は**Zero Diffを維持するための変更**） |
| **Z-3** | main.py Zero Diff（バイト単位で差分なし） |
| **Z-4** | Message Zero Diff（4型とも同一文字列を凍結） |
| **Z-5** | Chaining Zero Diff（`raise ... from None`により`__cause__`／`__context__`が到達不能） |
| **Z-6** | Signature Zero Diff（`__init__(message, reason)`・`reason`は既定値なしの必須引数のまま） |
| **Z-7** | Success-path Zero Diff（`GeneratedImage`生成経路が不変） |
| **Z-8** | CONTINUE Set Zero Diff（`_CONTINUABLE_REASONS`が4値のまま） |

Enum追加とpolicy更新は**不可分**であり、片側のみの実装・rollbackは禁止する。
policy未更新の場合は`ZERODIFF-SDK-CATEGORY`／`POLICY-CATEGORY`／`REJECTSET-EXACT`／
`ZERODIFF-PARTIAL-ENUM-ONLY`が同時FAILする。Enum未追加の場合は
`_REJECTED_REASONS`の定義時に`AttributeError`となりmodule import段階で全体FAILする。

### baseline固定guard（GR-1〜GR-11）

v6.21.0／v6.22.0のbaseline固定guardは、GR-9（保護対象パスへ触れるReleaseは
それ以前のすべてのbaseline固定guardのallow-listを更新する）に従い
allow-listとラベルのみを更新した。`_protected_paths`（22件）・`BASELINE_COMMIT`は
いずれも**不変**であり（GR-1・GR-2）、assertion件数も変わっていない（GR-5）。
ラベルからは許容件数の表現を除去した（GR-7）。

v6.23.0はGR-6に従い、自身のbaseline（`8fd845348d1ee4c80db8de2942da5f99c2bcf0fd`）を
固定した専用guardを持ち、`src/openai_image_generation`と
`src/image_generation_fallback_policy`の2パスについて
**equality（containment `changed ⊆ allowed` ∧ coverage `allowed ⊆ changed`）**を
検査する。これにより`__init__.py`を誤って変更した場合も機械的に検出される。

### 本Releaseの対象外

`UNKNOWN`の2経路分離・`INVALID_RESPONSE`の細分化（いずれもDI-11後半）、
`CONTENT_POLICY_REJECTED`の新設（例外型だけでは判定不能であり、response bodyの
`code`解析を要するため解析禁止contractに抵触する）、CONTINUE対象の拡大
（ORD-3の領域）、exception messageの改訂、`status_code`の属性公開、
observability／logging（DI-5）、retry／idempotency（DI-6）、
orphan media cleanup（DI-7）は、いずれも本Releaseの対象外である。

### Deferred（本Release時点）

- **S2R-1（DEF-6.22-15）：本Releaseで完了**
- **M5-1：継続**（本Releaseは構築形guardを必要とせず判断機会が発生しなかった）
- **DEF-6.23-1〜11：継続**（message改訂／CONTINUE拡大／DI-11後半／
  Content Policy判別／`status_code`公開／DI-5へのreason記録／
  zero-diff guardの共有レジストリ化／NOPARSE guardの他関数展開／
  v6.19の件数埋め込みScenario ID改名 ほか）
- **DEF-6.23-12（新規・Release Review RR-M-1由来）**：v6.23 E2Eの
  NOIMPACT陽性対照2件を実`_changed`／allow-list値に基づく検証へ置き換えるかを、
  次にv6.23のNOIMPACT guardへ触れるReleaseで判断する。本Releaseでは修正しない
- **DEF-TR-1：破棄済み**（deny-list方式の廃止により論点ごと消滅）

詳細は`docs/design/openai_image_generation_api_rejection_reason_classification_foundation.md`
（Architecture Design・Architecture Review・Test Review 初回／再判定・
Test Review Findings反映・Production Implementation・限定テスト・
Formal Regression・Release Review・Documentation Integrationの経緯を含む）を参照。

> **v6.24.0 追記**：本層が確立した`OpenAIImageGenerationErrorReason`（13値）は、
> v6.24.0で**末尾へ2値追加（13値→15値）された以外は一切変更していない**
> （v6.24.0のE2E `API-PREFIX-13-UNCHANGED`で先頭13値のname・value・順序の不変を、
> `ASTEQ-CLASSIFY-EQUAL`で`_classify_api_error()`のbaselineとのAST完全一致を、
> それぞれ確認済み）。
> 上記Deferredのうち**DEF-6.23-3・DEF-6.23-4はv6.24.0で完了**した
> （`UNKNOWN`のgeneric except Exception経路のみ`UNEXPECTED_EXCEPTION`へ、
> `INVALID_RESPONSE`のdata／b64_json構造不正2経路のみ`INVALID_RESPONSE_STRUCTURE`へ、
> それぞれ分離。catch-all経路・Base64系2経路はいずれも既存reasonのまま維持）。
> **DEF-6.23-12もv6.24.0で完了**した（NOIMPACT陽性対照2件を実`_changed`／
> allow-list値に基づく検証へ置換。既存containment／coverage／equality検査は
> 弱めていない）。**これによりDI-11は前半（v6.23.0）＋後半（v6.24.0）で完結した。**
> **M5-1は継続**（v6.24.0も構築形guardを必要とせず判断機会が発生しなかった）。
> **DEF-6.23-10はvalidator部分（`_validate_response_structure()`）のみv6.24.0で
> 解消し、他の入力受け取り関数への一般化は継続**する。
> 詳細は次節「OpenAI Image Generation Unknown and Invalid Response Reason
> Refinement Foundation層」。

---

## OpenAI Image Generation Unknown and Invalid Response Reason Refinement Foundation層（`src/openai_image_generation/`、v6.24.0）

> **本節はDocumentation Integration完了時点の記録である。Architecture Reviewは
> Approved with Minor Amendments（Minor 3件・Suggestion 1件・実装前必須Gate 1件、
> いずれも設計書へ反映）で完了し、Production実装前Gate確認（Gate 1／2／4／5：PASS、
> Gate 3：AST実測によりRequired Amendment 1件を検出・反映しPASS）を経て
> Production Implementationへ進行した。** 新規E2E（standalone script形式、
> `tests/test_e2e_v6_24_0_openai_image_generation_unknown_and_invalid_response_reason_refinement_foundation.py`、
> API-／UNK-／RESP-／B64-／SPLIT-／COMPAT-／POLICY-／CONT-／ZERODIFF-／
> POLICYFILE-／ASTEQ-／NOVALPARSE-／CHAIN-／MSG-／SIG-／DEP-／COMPATAPI-／
> NOIMPACT-／SOCKET-／ENV- の各Scenario prefix、**346アサーション、346/346 PASS**、
> 終了コード0）・累積Formal Regression（正式Inventory**27ファイル**：
> `test_e2e_v1_11_0_save_result.py`・`test_e2e_v5_9_0_*.py`・
> `test_e2e_v6_0_0_*.py`〜`test_e2e_v6_24_0_*.py`、**既存26ファイル4072/4072 PASS＋
> 新規v6.24.0 346/346 PASS＝総合4418/4418 PASS**、FAIL 0・SKIP 0・終了コード異常0・
> 外部接続0）とも完全PASSした。**既知差分は`test_e2e_v6_19_0_*.py`の262→272（+10）と
> `test_e2e_v6_23_0_*.py`の332→341（+9）のみ**であり、v6.11.0（248）・v6.21.0（147）・
> v6.22.0（324）はいずれも件数不変、他21ファイルもv6.23.0実績から変動なし。
> `test_e2e_v6_23_0_*.py`の+9は設計時点の宣言（+3、DEF-6.23-12解消分のみ）を
> 実測（限定テスト）で訂正した値であり、同ファイルが`test_e2e_v6_19_0_*.py`と
> 同型の`_ALL_REASONS`駆動ループを2箇所持つことに伴う自動増（+6）が
> 設計時点で計上漏れであったことによる。**期待値の緩和ではなく、coverage対象が
> 13値から15値へ拡張されたことによる構造的追随**である。限定テスト実測値と
> Formal Regression実測値は完全に一致し（乖離0）、Documentation Integrationを経て
> 現時点に至る。Release Reviewは初回**Changes Required**（Blocking 0／Major 2／
> Minor 3／Suggestion 4）からFindings Amendment反映・再確認（Minor 1件検出・反映）を
> 経て、再々確認で**最終Verdict：Approved with Suggestions**（Blocking 0／Major 0／
> Minor 0／Suggestion 6）が確定した。Code Reviewでは**Approved with Suggestions**
> （Blocking 0／Major 0／Minor 0／Suggestion 7）を記録した。**Finalizeでs-6を解消した
> ため、現在残存する非ブロッキングSuggestionは6件（s-1〜s-5、s-7）である。** Code
> Review時点のVerdictおよび件数は履歴として維持する。
> 人間による設計・実装・E2E・文書の最終承認は**完了**している。
> **commit・pushはいずれも未実施のままである。**

### Purpose

v6.23.0時点で1値へ集約されていた2組の失敗経路を、reasonとして分離する（DI-11後半）。
`UNKNOWN`は「`openai.APIError`のcatch-all経路」と「`generate()`のgeneric
`except Exception`経路」の2つを、`INVALID_RESPONSE`は「`data`／`b64_json`構造不正」と
「Base64デコード失敗／0バイト」の2つを、それぞれ1つのreasonへ集約していた。
本Releaseは**新しいreasonを発明するのではなく、既にmessage単位では区別されていた
境界をreason値へ昇格させる**ことが目的であり、**判断（CONTINUE／PROPAGATE）は
一切変更しない**。

```
openai SDK 例外                              response構造
        │                                            │
        ▼                                            ▼
_classify_api_error(exc)                _validate_response_structure(response)
        │                                            │
        ├─ 既存10型 → 既存10 reason（不変）          ├─ data／b64_json構造不正
        ├─ openai.APIError catch-all → UNKNOWN（不変）│    → INVALID_RESPONSE_STRUCTURE ★v6.24.0
        │                                            └─ 正常時 → (None, None, b64_json)（不変）
        ▼
generate()
        │
        ├─ except openai.APIError as exc: _classify_api_error(exc)（不変）
        └─ except Exception: → UNEXPECTED_EXCEPTION ★v6.24.0
                （旧: UNKNOWN。catch-allのUNKNOWNとは別reasonへ分離）
        │
        ▼
OpenAIImageGenerationError(message【凍結】, reason)
        │
        ▼
decide_image_generation_fallback(error)        ← v6.19.0（無改修）
        │
        ├─ reason ∈ _REJECTED_REASONS（5値・不変）
        ├─ reason ∈ _CONTINUABLE_REASONS（4値・不変）
        └─ その他（UNEXPECTED_EXCEPTION・INVALID_RESPONSE_STRUCTURE含む）
               → UNCLASSIFIED / PROPAGATE_ORIGINAL_ERROR
```

### Package Boundary

**新規packageは作成しない。既存1packageの1ファイルのみを変更する。**
`image_generation_fallback_policy.py`は**無改修（0 diff）**である
（新2 reasonがいずれもallow-list方式により自動的にUNCLASSIFIEDへ落ちるため）。

```
src/openai_image_generation/
    openai_image_generator.py        ← 変更（Enum 2値追加・reason差し替え2箇所）
    __init__.py                      ← 無変更（新symbolなし。__all__は3 symbolのまま）

src/image_generation_fallback_policy/
    image_generation_fallback_policy.py  ← 無変更（0 diff）
    __init__.py                      ← 無変更
```

### reason taxonomy（13値 → 15値）

既存13値の**name・value・定義順は1文字も変更していない**。新2値は末尾へ追加する。

| # | name | value | 由来 |
|---|---|---|---|
| 1〜13 | 既存13値 | 既存 | 既存（不変。v6.11.0〜v6.23.0で確立） |
| 14 | **`UNEXPECTED_EXCEPTION`** | `unexpected_exception` | `generate()`のgeneric `except Exception`経路 |
| 15 | **`INVALID_RESPONSE_STRUCTURE`** | `invalid_response_structure` | `_validate_response_structure()`の`data`／`b64_json`構造不正 |

`UNKNOWN`・`INVALID_RESPONSE`を削除しない理由は、①`__all__`経由で公開された
Public APIであること、②既存E2Eが直接構築していること、③外部から構築した場合の
下流の結論は従来と完全に同一であること、である。**両値とも production から
到達し続ける（catch-all経路・Base64系経路が残るため）**が、到達する経路の範囲は
縮小する。`INVALID_RESPONSE`の名称と縮小後の適用範囲の乖離はIL-6.24-1として
後方互換上のInherited Limitationのまま受容し、改名判断はDEF-6.23-1へ合流させた。

### 分類契約（安全側判定・positive allow-list guard）

`_classify_api_error()`は**本Releaseで一切変更していない**（baselineとAST完全一致。
I-EXC-1は無改修のまま有効）。`_validate_response_structure()`の外部応答値読み取りは
**I-VAL-1**（新設）で規律される。

```
【I-VAL-1】_validate_response_structure() の関数本体において、外部応答値に
          由来する識別子集合 R を根とする ast.Attribute が0件であること。

          R は第1 positional parameter（response）を初期値とし、getattr戻り値の
          代入先を再帰的に加える不動点集合として導出する（本Releaseの実測値：
          R = {response, data, b64_json}）。

          R に属さない識別子（内部Enumメンバ参照等）への属性アクセスは
          対象外であり違反にならない。
```

Production実装前Gate確認のAST実測で、**関数全体で`ast.Attribute`が0件**という
旧文言が、`_validate_response_structure()`が正当に行う内部Enum参照
（`OpenAIImageGenerationErrorReason.INVALID_RESPONSE`等、実測2件）を
誤って違反判定する欠陥を発見し、Rスコープ限定へ訂正した（許容対照W-5、
陽性対照12形・陰性対照5形で機械的に保証）。`getattr`は
`getattr(response, "data", _MISSING)`／`getattr(data[0], "b64_json", _MISSING)`の
2件のみ、各3位置引数・keyword 0件、第2引数は`{"data","b64_json"}`のいずれか、
という既存契約は維持する。

### fallback policyの写像（無改修）

新2 reason（`UNEXPECTED_EXCEPTION`／`INVALID_RESPONSE_STRUCTURE`）は
`_REJECTED_REASONS`にも`_CONTINUABLE_REASONS`にも属さないため、
**`image_generation_fallback_policy.py`を1バイトも変更せずに**
`UNCLASSIFIED`＋`PROPAGATE_ORIGINAL_ERROR`へ自動的に写像される
（v6.19.0のC-17性質：allow-list方式であるため新値は自動的に安全側へ落ちる）。
category splitは**4/5/2/2 → 4/5/2/4**となる。
...
> public属性`OpenAIImageGenerationError.reason`は対象2経路に対して**意図的に別の値に
> なる**ため、Production behavior全体が不変であるとは記載できない。

主張するZero Diffは次の8点に限定される。

| ID | 内容 |
|---|---|
| **Z-1** | Runtime Action Zero Diff（15 reasonすべてで`action`がv6.23.0時点と一致） |
| **Z-2** | Category Zero Diff（`decision.category`が一致。policy側は0 diffのため自動的に成立） |
| **Z-3** | main.py Zero Diff（バイト単位で差分なし） |
| **Z-4** | Message Zero Diff（全raise経路のmessage文字列が完全一致） |
| **Z-5** | Chaining Zero Diff（`raise ... from None`により`__cause__`／`__context__`が到達不能） |
| **Z-6** | Signature Zero Diff（`_validate_response_structure()`の3要素返却形を含め不変） |
| **Z-7** | Success-path Zero Diff（`GeneratedImage`生成経路が不変） |
| **Z-8** | CONTINUE Set Zero Diff（`_CONTINUABLE_REASONS`が4値のまま） |

### baseline固定guard（GR-1〜GR-11）

v6.21.0／v6.22.0／v6.23.0のbaseline固定guardは、GR-9に従いallow-listとラベルのみを
更新した。`_protected_paths`（22件）・`BASELINE_COMMIT`はいずれも**不変**である。
v6.24.0はGR-6に従い、自身のbaseline（`38e2487db5760034f4a994319350244960a42e1b`）を
固定した専用guardを持ち、`src/openai_image_generation`の1パスについて
**equality（containment ∧ coverage）**を検査する。Production source allow-list
（1ファイルのみ）とtest change allow-list（6ファイル）は別変数として分離した。

### 本Releaseの対象外

Content Policy拒否の判別（`CONTENT_POLICY_REJECTED`の新設。response bodyの`code`解析を
要し解析禁止contractに抵触）、CONTINUE対象の拡大（ORD-3の領域）、exception messageの
改訂（IL-1・IL-6.24-1）、`status_code`の属性公開、observability／logging（DI-5）、
retry／idempotency（DI-6）、orphan media cleanup（DI-7）、positive allow-list方式guardの
他の入力受け取り関数（`_build_generated_image()`・`generate()`等）への一般化は、
いずれも本Releaseの対象外である。

### Deferred（本Release時点）

- **DEF-6.23-3／DEF-6.23-4：本Releaseで完了**
- **DEF-6.23-12：本Releaseで完了**（実値ベースの陽性対照D12-1〜D12-5へ置換）
- **DI-11：本Releaseで完結**（前半＝v6.23.0、後半＝v6.24.0）
- **DEF-6.23-10：validator部分のみ本Releaseで解消**。他関数への一般化は継続
- **M5-1：継続**（本Releaseも構築形guardを必要とせず判断機会が発生しなかった）
- **DEF-6.23-1／2／6／7／8／9／11：継続**（message改訂・CONTINUE拡大・
  Content Policy判別・`status_code`公開・DI-5へのreason記録・zero-diff guardの
  共有レジストリ化・v6.19の件数埋め込みScenario ID改名。**Release Review Amendment・
  第5回改訂で訂正：`NOPARSE guardの他関数展開`はDEF-6.23-10の説明であり本7件には
  含まれない。上のDEF-6.23-10の行を参照**）
- **IL-6.24-1（新規）**：`INVALID_RESPONSE`の名称と範囲縮小後の適用範囲の乖離を
  後方互換上のInherited Limitationとして受容し、改名判断はDEF-6.23-1へ合流させた
- DI-5／DI-6／DI-7／DI-9／DEF-6.22-1：本Release対象外。既存の状態を維持

**Non-Blocking Suggestions（Code Review時点7件・現在残存6件）**：Release Review・
Code Reviewとも必須Findingは0件である。Code ReviewではSuggestion 7件を記録した。
Finalizeでs-6（Release Review状態記載の3文書間不一致）を解消したため、現在残存する
非ブロッキングSuggestionは6件（s-1〜s-5、s-7：signature検証の将来拡張・NOVALPARSE
陽性対照の強化・D12-3相当の冗長性整理・Production長行の折り返し・設計書内相対表現の
字義的不整合・I-VAL-1陰性対照W-1／W-2の重複）であり、いずれも本Releaseでは修正しない。
Code Review時点のVerdictおよび件数は履歴として維持する。詳細は設計書19.6節〜19.8節。

詳細は`docs/design/openai_image_generation_unknown_and_invalid_response_reason_refinement_foundation.md`
...
> **2026-09-13追記**：本節記載時点（v6.29.0）ではFuture Extensionだった
> 「Retry Observability Runtime Wiring」「CLIの本Facadeへの委譲統一」は、
> いずれもRelease 6.33（Retry Observability Runtime Integration）で完了した
> （下記「Retry Observability Runtime Integration層」節を参照）。ただし
> v6.33設計時点でRuntime配線先は`scripts/run_retry_runtime.py`（Runtime
> Entry Point）であることが確定しており、実際には`RetryRuntimeOrchestrator`
> 自体・`RetryCompositionRoot`自体はいずれも無改修のまま維持された（新規
> package`src/retry_runtime_observability/`を介した配線としたため。詳細は
> v6.33設計書AD-2）。以下は当時の記述として保持する。
...
`RetryObservabilityPipeline`（v6.29.0）のRetry Runtime実配線（6.33、Retry Observability Runtime Integration）は、v6.33.0で完了した（下記「Retry Observability Runtime Integration層」節参照）。generic non-HRR automatic retry redispatch等、本Amendmentが明示的にOut of Scopeとした事項は本Releaseで解決済みではない。`release_claim()`戻り値未確認によるdiagnostic gapはnon-blocking suggestionとして記録済み（v6.33.0でも引き続きOut of Scope・未対応）。
...
## Retry Observability Runtime Integration層（`src/retry_runtime_observability/`、v6.33.0 実装完了）

> **本節は実装完了時点の記録である。新規E2E（`test_e2e_v6_33_0_retry_observability_runtime_integration_foundation.py`）はv6.33 E2E PASS（real `.run/retry_runtime.lock`の有無によりアサーション総数が186件または187件のいずれかになる条件分岐を含むため、固定数では表記しない）。既存v6.8.0 CLI Wiring E2E（monkeypatch対象再配置後）は197/197 PASS。Architecture Design（`docs/design/retry_observability_runtime_integration_foundation.md`）はCodex `codex-readonly-review` workflowによる独立read-only Architecture Reviewを4ラウンド実施し、Round 4で`APPROVED`（Blocking 0／Major 0／Minor 0／Suggestions 0）に収束した。実装完了後のIndependent Code Review（Codex read-only）はRound 1で`NOT APPROVED`（Blocking 0／Major 2／Minor 3）を検出し、限定修正を実施した（詳細は`docs/CHANGELOG.md` [v6.33.0]エントリ参照）。正式Formal Regression（全Inventory）は本節時点では未実施（別フェーズで実施予定）。**

`RetryObservabilityPipeline`（v6.29.0で完成した消費者不在のOrchestration/Facade）を実際のRetry Runtime（`scripts/run_retry_runtime.py`）へ配線し、`scripts/show_retry_notification.py::build_report()`が保持し続けていた合成ロジックの重複を解消した、v6.29.0設計書14章が定義した「将来Wiring境界」の実施。

### 新規package `src/retry_runtime_observability/`

`RetryRuntimeObservabilityReporter`のみを公開する葉パッケージ。`observe()`はログ全件の読み取り（`RetryRuntimeLogReader.read()`）とPipeline評価（`RetryObservabilityPipeline.evaluate()`）のみを行うraw契約であり、例外を一切捕捉せずそのまま伝播する（単体テストでの契約検証を優先）。`observe_and_report()`は、読み取り・評価・整形（呼び出し元が注入するformatter）・コンソール出力（`print()`）の全段階をひとつの失敗包含境界として実行する唯一の入口であり、`Exception`派生の例外はすべてcontainしてstderrへWARNINGを出力する一方、`BaseException`（`SystemExit` / `KeyboardInterrupt` / `GeneratorExit`）は対象外としそのまま伝播させる（Loop実行中のGraceful Shutdown割込みを誤って握りつぶさないため）。WARNING出力自体（メッセージ整形・`print()`呼び出し）の失敗も、内部のbest-effortヘルパー`_warn_best_effort()`でcontainし、呼び出し元へは一切伝播しない。`retry_metrics`と`retry_observability_pipeline`のみに依存し、`retry_engine` / `retry_queue` / `retry_history` / `retry_lineage` / `retry_enqueue_trigger` / `scheduler` / `retry_composition` / `retry_runtime_orchestrator` / `scripts/`のいずれにも依存しない（新規AST Guardで逆依存禁止を機械的に保証）。

### Full-History Semantics（windowingの不採用）

Runtime・CLIのいずれも`RetryRuntimeLogReader.read()`が返す全件を`RetryObservabilityPipeline.evaluate()`へそのまま渡す。直近N件のみを評価する「Bounded Tail Window」案はArchitecture Design検討時に一度採用したが、Codex Round 1 Architecture Reviewで「Runtime側の判定入力だけを変え、CLI側は全件のままになるため、records調達方式の域を超えた新しいobservability policyの導入に相当する」とMAJOR指摘を受け撤回した。`RetryMetricsCalculator`が前提とするcross-cycle cumulative semantics（v6.3.0以来の既存契約）はRuntime側の観測でも変更していない。

### Current-Cycle Inclusion Contract（`RetryRuntimeCycleLogger.log_cycle()`のbool化）

`RetryRuntimeCycleLogger.log_cycle()`（v6.2.0）の戻り値を`None`→`bool`へ追加的に変更した（`True`＝書き込み処理がOSErrorを送出せず完了、`False`＝OSError捕捉）。fsync durabilityや書き込み完了後の外部変更からの保護は保証しない、限定的な契約として明記した。`scripts/run_retry_runtime.py`の`run_cycle()`は、この戻り値が`True`の場合のみ`RetryRuntimeObservabilityReporter.observe_and_report()`を呼び出す。`False`の場合は当該cycleのobservability出力を単純にskipし、stale historyを当該cycleのreportとして評価しない。既存caller・既存テストが戻り値`None`に依存していないことをread-only確認済みであり、既存契約を破壊しない追加的変更として扱った。

### CLI Delegation（`scripts/show_retry_notification.py::build_report()`）

CLIローカルで直接インスタンス化していた`RetryHealthEvaluator` / `RetryAlertEvaluator` / `RetryNotificationEvaluator` / `RetryNotificationMessageBuilder`を削除し、`RetryObservabilityPipeline().evaluate()`への薄い委譲へ統一した。`RetryRuntimeLogReader`の直接呼び出しは維持し、`format_report()`のシグネチャ・出力形式は完全に無変更のまま保った。既存v6.8.0 CLI E2Eがクラスレベルでmonkeypatchしていた4シナリオ（Message Builder呼び出し回数・ValueError/RuntimeError伝播）は、patch対象を実際にクラス参照を解決する`retry_observability_pipeline.retry_observability_pipeline`モジュールへ再配置し、検証するアサーション自体は無変更のまま維持した。

### Architecture Reviewの経緯（Codex独立read-only review、Round 1〜4）

Architecture DesignはRound 1〜3で累計MAJOR 6件・MINOR 5件（Bounded Tail Windowという未承認のobservability policy変更の撤回・failure containment境界の拡張とWARNING出力自体のbest-effort化・current-cycle inclusion契約の確立・v6.8 CLI monkeypatchテストの移行戦略・zero-diff registryのsource contribution明記等）を検出・解消し、Round 4で`APPROVED`（Blocking 0／Major 0／Minor 0／Suggestions 0）に収束した。実装完了後のIndependent Code Review（Round 1）では、新規E2Eが実project直下の`.run/retry_runtime.lock`を直接削除していたテスト分離不備（MAJOR）・documentation未更新（MAJOR）等を検出し、`RetryRuntimeLock`をFakeへ差し替える既存注入seamの活用・本節を含むdocumentation整合により対応した。

### Test Review・Regressionの実績

新規E2E（`tests/test_e2e_v6_33_0_retry_observability_runtime_integration_foundation.py`）はv6.33 E2E PASS（real `.run/retry_runtime.lock`の有無でアサーション総数が186件・187件のいずれかになる条件分岐を含む）。既存v6.8.0 CLI Wiring E2E（monkeypatch対象再配置後）は197/197 PASS。限定関連回帰（retry系44ファイル）を実施し、Release 6.33が変更した`src/retry_runtime_logging` / `scripts/run_retry_runtime.py`に依存する既存Architecture Guard3ファイル（`test_e2e_v5_5_0` / `test_e2e_v6_3_0` / `test_e2e_v6_4_0`）へ狭い除外編集を適用した上で全PASSを確認した（`docs/CHANGELOG.md` [KI-32]参照）。正式Formal Regression（全Inventory）は本節時点では未実施（別フェーズで実施予定）。
