...
### [KI-33] Release 6.34（Scheduler Driver & Duplicate Dispatch Safety）実装フェーズ完了時点で新設`scripts/run_scheduler_driver.py`がuncommittedのため、複数の既存Architecture Guard（`test_e2e_v6_22_0`・`v6_23_0`・`v6_24_0`・`v6_26_0`・`v6_27_0`）が`"untracked集合が空である"`系チェックでFAILする（commitにより自然解消する既知差分、`[KI-3]`系列と同型）

- **発見日**：2026-09-16（Release 6.34.0 Implementation Phase、Formal Regression実施時）
- **対象**：`tests/test_e2e_v6_22_0_wordpress_media_upload_failure_reason_classification_foundation.py`（`NOIMPACT-NO-UNTRACKED[scripts]`、1件FAIL、323/324 PASS）・`tests/test_e2e_v6_23_0_openai_image_generation_api_rejection_reason_classification_foundation.py`（`NOIMPACT-NO-UNTRACKED[scripts]`・`NOIMPACT-SCOPE-COVERAGE[scripts]`・`NOIMPACT-SCOPE-EXACT[scripts]`、3件FAIL、352/355 PASS）・`tests/test_e2e_v6_24_0_openai_image_generation_unknown_and_invalid_response_reason_refinement_foundation.py`（同型3件FAIL、359/362 PASS）・`tests/test_e2e_v6_26_0_zero_diff_guard_registry_foundation.py`（上記3ファイルを子プロセスとして実行するRUNTIME-*系チェックの伝播、8件FAIL、256/264 PASS）・`tests/test_e2e_v6_27_0_image_generation_gate_value_validation_foundation.py`（自身のZERODIFF-2[scripts]1件＋上記3ファイルの伝播9件、計10件FAIL、109/119 PASS）
- **原因**：Release 6.34.0はArchitecture承認（`docs/design/scheduler_driver_duplicate_dispatch_safety_foundation.md`、Codex `codex-readonly-review` Round 5 APPROVED）に従い、新規`scripts/run_scheduler_driver.py`を追加し、`scripts/run_workflow_engine.py`を変更した。`tests/zero_diff_guard_registry.py`の`_SOURCE_CHANGE_CONTRIBUTIONS`へ`("scripts", "v6.34.0", {"scripts/run_workflow_engine.py", "scripts/run_scheduler_driver.py"})`として正しく登録済みであり、これにより共有レジストリを参照する`allowed_source_changes_for()`ベースの`NOIMPACT-SCOPE[...]`（baselineからの差分がallow-listの範囲内であること）チェック自体は正しくPASSしている。しかし本Release完了時点では`commit`が禁止されているため（Human Gate待ち）、`scripts/run_scheduler_driver.py`は`git`上**untracked**のまま残っている。`git status --porcelain --untracked-files=all`ベースの`NOIMPACT-NO-UNTRACKED[scripts]`系チェック（GR-9の対象外、registryのallow-listを持たない恒久的な「untrackedファイルはゼロであること」という別種の検査）は、この性質上、承認済みの新規ファイルであっても**commitされるまで必ずFAILする**。加えて、`_allowed_source_changes_for()`が返す許容集合（v6.23.0以降の全window寄与のunion、`scripts/run_scheduler_driver.py`を含む）と、`git diff <baseline>`が返す実差分集合（untrackedファイルは`git diff`に現れないため`scripts/run_scheduler_driver.py`を含まない）の非一致により、`NOIMPACT-SCOPE-COVERAGE[scripts]`・`NOIMPACT-SCOPE-EXACT[scripts]`（allow-list宣言済みファイルが実際に変更されていることの逆方向チェック）も同じ理由でFAILする。`[KI-3]`〜`[KI-32]`が繰り返し記録してきた「`git diff --quiet`ベースのチェックはcommit後に自然解消する」という既知パターンの、untracked-file検査版である
- **対応状況**：対応しない（対応不能。commit自体がRelease 6.34.0 Implementation Phaseのタスク範囲外、Human Gate対象のため）。上記5ファイルの個別ファイル修正は行っていない。実際にsrc/scripts側のコード自体に欠陥がないことは、共有レジストリベースの`NOIMPACT-SCOPE[scripts]`（baseline差分がallow-list範囲内）チェック自体がPASSしていることで別途確認済み。またRelease 6.34.0自身の新規E2E（`test_e2e_v6_34_0_scheduler_driver_duplicate_dispatch_safety_foundation.py`、55/55 PASS）・Invariant #35 closure oracle（`test_e2e_v6_32_7_invariant_35_closure_oracle.py`、新設10番目のSIDE_EFFECT_CAPABLEエントリとしてAST closure実測で`sinks={A,B,C}`・`unresolved=[]`を確認済み、54/54 PASS）はいずれも正常にPASSしている
- **今後の対応**：本Release 6.34.0がcommitされた時点で、上記5ファイルの該当チェックは`[KI-3]`等と同様に自然解消する見込み（`scripts/run_scheduler_driver.py`がtrackedファイルになり、`git status --porcelain --untracked-files=all -- scripts`の出力が空になるため）。commit後のFormal Regression再実行で0 FAILになることを確認すること
- **post-commit追記（2026-09-16、本Release 6.34.0 commit後のFormal Regression再実行時）**：`test_e2e_v6_22_0`・`v6_23_0`・`v6_24_0`・`v6_26_0`の4ファイルは上記見込みどおりcommit後に自然解消し、全PASSを確認した。`test_e2e_v6_27_0_image_generation_gate_value_validation_foundation.py`のみ、本エントリが想定していた`untracked`起因のチェックとは別の`ZERODIFF-1[scripts]`（`RELEASE_START_HEAD`からの`git diff`が承認済み例外集合に収まることを確認する、`git status`ベースではないチェック）で新たに1件FAILした（118/119 PASS）。原因は、共有レジストリ`tests/zero_diff_guard_registry.py`の`_SOURCE_CHANGE_CONTRIBUTIONS`へは`scripts/run_scheduler_driver.py`が正しく`"v6.34.0"`として登録済みだった一方、`test_e2e_v6_27_0`自身が個別に持つ`_ZERODIFF1_ALLOWED_EXCEPTIONS["scripts"]`（本registryを参照しないstandalone guard）への反映が漏れていた、`[KI-32]`と同型の historical guard mapping漏れ（本KI-33の対象5ファイルの原因とは別種）だった。`test_e2e_v6_27_0`の`_ZERODIFF1_ALLOWED_EXCEPTIONS["scripts"]`へ`scripts/run_scheduler_driver.py`をexact-path例外として追加し、`_TEST_CHANGE_CONTRIBUTIONS`へ`("test_e2e_v6_27_0_image_generation_gate_value_validation_foundation.py", "v6.34.0")`を追記する狭いscopeの補正で解消した（他ファイル・他pathへの変更なし）。commit/push/amendは本補正には含まれない

---

## [v6.34.0] - 2026-09-16 ★ Scheduler Driver & Duplicate Dispatch Safety

> `docs/MVP_COMPLETION_ROADMAP.md`が定める6.34スコープに対応し、`SchedulerEngine`（v2.6.0）のpure性を維持したまま、production workflow（`WorkflowEngineManager.run()`経由のNEWS/REVIEW/PUBLISH）を定期駆動できる、再起動・クラッシュ・多重driverに対しても安全なloop driverを新設した。
>
> Architecture DesignはClaude Code単独設計→Codex `codex-readonly-review`による独立read-only reviewを5ラウンド実施して収束した：Round 1（Blocking 2／Major 4／Minor 2）→Round 2（Blocking 2／Major 2／Minor 1）→Round 3（Blocking 2／Major 2／Minor 2）→Round 4（**Blocking 0**／Major 1／Minor 2／Suggestions 1）→Round 5（**APPROVED**、Blocking 0／Major 0／Minor 1／Suggestions 1）。26章のArchitecture Gate Checklist（9項目）はユーザーが個別にACCEPT/確認済み（Gate 9のManual Recovery専用CLIは本Releaseから除外）。
>
> 実装完了後のIndependent Code Review（同workflow、read-only）は、`src/scheduler_dispatch_ledger/scheduler_dispatch_ledger_store.py`を中心に6ラウンド実施した：Round 1（Blocking 1：store_dir不在時の`list_all()`/`get()`が「recordなし」と誤認しうる／Major 1：record schema検証不足）→Round 2（Blocking 1：`Path.exists()`/`Path.glob()`がPython 3.14でOSErrorを握りつぶしうる）→Round 3（Blocking 1：TOCTOU残存・symlink追従の未対応）→Round 4（**Blocking 0**、Major 1：`_sanitize_event_identity()`の非単射性によるファイル名衝突リスク）→Round 5（Major 1：Windowsファイルシステムのcase-insensitivityに対する単射性欠如）→**Round 6（APPROVED、Blocking 0／Major 0／Minor 1 cosmetic）**。全6ラウンドの指摘はいずれもArchitecture契約を弱めずに解消し、各修正を直接検証する新規回帰テスト（テスト29〜34）を追加した。

### Added

- `src/scheduler_dispatch_ledger/` 新規パッケージ：`DispatchPhase`（3-phase: CLAIMED/CONFIRMED/RECOVERY_REQUIRED、owner_token authorityなし）・`DispatchLedgerEntry`・`SchedulerDispatchLedger`（`claim()`/`confirm()`/`reconcile_stale_claims()`/`list_recovery_required()`/`peek()`）・`SchedulerDispatchLedgerStore`抽象＋`JsonSchedulerDispatchLedgerStore`実装（`os.lstat()`/`os.scandir()`ベースのfail-closed読み取り、symlink拒否、単射なファイル名エスケープ、filename/event_identityクロスチェック）・`SchedulerDispatchLedgerStoreLock`（`RetryLineageStoreLock`と同型の短時間リトライ付き排他）・`SchedulerDispatchLedgerConfig`。Retry系パッケージを一切importしない独立package。
- `src/scheduler_schedule_source/` 新規パッケージ：`ProductionScheduleSource`（production `SchedulerJob`定義の唯一の供給元。MVP初期実装は`scripts/run_workflow_engine.py`が保持していた`job_id="workflow_engine_demo_daily"`のDAILY 09:00 Job定義を移設したもの、job_id等の値は完全に維持）。
- `src/scheduler_driver/` 新規パッケージ：`SchedulerDriverConfig`（`SCHEDULER_DRIVER_ENABLED`ゲート、デフォルトfalse）・`SchedulerDriverCompositionRoot`（`SchedulerEngine`/`ProductionScheduleSource`/`SchedulerDispatchLedger`/`WorkflowEngineManager`を環境変数から組み立て）・`SchedulerDriverOrchestrator`（`run_once()`：Ownership-Integrity Self-Check→`reconcile_stale_claims()`→`SchedulerEngine.evaluate()`（無改修）→Design Decision Jのretry候補除外filter→per-event fail-closed claim→dispatch→confirm）・`LockIntegrityViolationError`/`_verify_lock_ownership()`/`_release_if_owned()`（Lock Lifecycle Contract）・`_select_dispatchable_events()`（`metadata["retry_candidate"]`不在を一次機構、`ProductionScheduleSource`のjob_id allowlistを二次防御とする2層フィルタ）・`build_event_identity()`（`(job_id, occurrence_minute)`の安定な組）。
- `scripts/run_scheduler_driver.py` 新規CLIエントリスクリプト：`RetryRuntimeLock`/`RetryRuntimeLoop`/`RetryRuntimeShutdown`（いずれも`src/retry_runtime_*/`、**コード変更ゼロ**のまま再利用）を用いたsingle-active-driver制約・loop駆動。`lock.release()`の唯一の呼び出し経路を`main()`の`finally`節（`_release_if_owned()`）へ集約するLock Lifecycle Contractを実装（起動時ownership検証は`try`ブロック内側の最初の文）。
- `tests/test_e2e_v6_34_0_scheduler_driver_duplicate_dispatch_safety_foundation.py` 新規作成（**55/55 PASS**）：stable event identity・fail-closed dispatch・duplicate suppression（restart/same-minute）・claim→dispatch crash・fresh-read-under-lock契約・second driver fail-closed・Ownership-Integrity Self-Check/Lock Lifecycle Contract・retry-candidate二層除外・exception containment・ledger store I/O失敗（store_dir消失・symlink置換・schema検証・単射性・case-insensitivity安全性）・Scheduler Driver/Retry Runtime構造的非干渉・`scripts/run_workflow_engine.py`のZero-Diff境界とR9 contract evidence、を網羅する。

### Changed

- `scripts/run_workflow_engine.py`：`build_demo_job()`（ローカル固定Job定義）を`ProductionScheduleSource().jobs()`へ置換（9.2章で確定した仕様どおり全件ループ登録）。`DEMO_JOB_ID`/`DEMO_JOB_SCHEDULE`定数は削除。`events[0]`のみを処理する既存挙動・`--job-id`手動経路は意図的に無変更のまま維持（複数event一括処理は新設`scripts/run_scheduler_driver.py`の責務、25章R9として明記）。
- `tests/test_e2e_v6_32_7_invariant_35_closure_oracle.py`：`SIDE_EFFECT_CAPABLE_MANIFEST`へ10番目のエントリとして`"scripts/run_scheduler_driver.py": frozenset({SINK_A, SINK_B, SINK_C})`を追記（`scripts/run_workflow_engine.py`と同一のsink到達性）。新規Stage B検証ブロック（`closure_of_script("scripts/run_scheduler_driver.py")`の実測）を追加し、declared値と実測値（`sinks={A,B,C}`、`unresolved=[]`）の一致を直接確認した（manifestへの追記のみに頼らない）。
- `tests/zero_diff_guard_registry.py`：`RELEASE_ORDER`へ`"v6.34.0"`を追記。`_SOURCE_CHANGE_CONTRIBUTIONS`へ`("scripts", "v6.34.0", {"scripts/run_workflow_engine.py", "scripts/run_scheduler_driver.py"})`を追記。`_TEST_CHANGE_CONTRIBUTIONS`へ新規E2E自身・本レジストリ自身・`test_e2e_v6_32_7_invariant_35_closure_oracle.py`（上記manifest追記に伴う変更）の計3件を`"v6.34.0"`として登録した。

### Zero-Diff対象（Architecture 21章、実装フェーズでも維持を確認）

`src/scheduler/`・`src/workflow_engine/`・`src/execution_history/`・`src/workflow_monitor/`・`src/retry_lineage/`・`src/retry_engine/`・`src/retry_queue/`・`src/retry_history/`・`src/retry_composition/`・`src/retry_enqueue_trigger/`・`src/retry_runtime_orchestrator/`・`src/retry_runtime_observability/`・`src/retry_observability_pipeline/`・`src/retry_runtime_logging/`・`src/retry_runtime_lock/`・`src/retry_runtime_loop/`・`src/retry_runtime_shutdown/`・`scripts/run_retry_runtime.py`・`side_effect_safety`/`protected_operation_manifest`関連パッケージはいずれも無改修（`git diff`で個別確認済み）。新規Scheduler Driverコードは上記いずれの`retry_*`パッケージもimportしない（`grep`による静的確認済み）。

### Test Review・Regressionの実績

新規E2E（`test_e2e_v6_34_0_scheduler_driver_duplicate_dispatch_safety_foundation.py`）は**55/55 PASS**。Invariant #35 closure oracle改訂後は**54/54 PASS**。Independent Code Review（Codex `codex-readonly-review`、read-only、6ラウンド）は**APPROVED**（Blocking 0／Major 0／Minor 1 cosmetic、doc-comment箇所のみ）。

限定関連回帰として、Release 6.30〜6.33の代表的E2E（`test_e2e_v2_6_0`〜`test_e2e_v2_9_0`・`test_e2e_v6_30_0`〜`test_e2e_v6_33_0`等）を個別実行し、Zero-Diff対象への影響がないことを確認した。

**正式Formal Regression**（正式Inventory37ファイル：`test_e2e_v1_11_0`＋`test_e2e_v5_9_0`＋`test_e2e_v6_0_0`〜`test_e2e_v6_34_0`。実測ベースで37ファイルを確認、過去エントリの記録「35ファイル」との差異は本Release時点の実地棚卸しによるものであり原因は未確定）を実施した。**32/37ファイルがexit code 0**。残る5ファイル（`test_e2e_v6_22_0`・`v6_23_0`・`v6_24_0`・`v6_26_0`・`v6_27_0`）は、本CHANGELOGの`[KI-33]`が記録する通り、**新設`scripts/run_scheduler_driver.py`が本Release完了時点でuncommittedであることのみに起因する既知差分**であり、commit後に自然解消する見込みである（`[KI-3]`系列と同型）。アサーション総数の実測：5940件中5915 PASS・25 FAIL（FAILはすべて`[KI-33]`に帰着）。**post-commit追記**：commit後の再実行で4ファイルは見込みどおり自然解消したが、`test_e2e_v6_27_0`のみ別種の`ZERODIFF-1[scripts]`（historical guard mapping漏れ）で1件FAILし、`[KI-33]`の post-commit追記が記録する狭い補正で解消した（詳細は`[KI-33]`本文参照）。

### Future Extension

- Manual Recovery Procedure用の専用CLIスクリプト（`list_recovery_required()`を消費する読み取り専用ツール）：Architecture Gate Checklist item 9でスコープ外と確定済み（`docs/design/scheduler_driver_duplicate_dispatch_safety_foundation.md` 20章）。
- `RECOVERY_REQUIRED`occurrenceの自動復旧：Out of Scope（Roadmap 6.34明記）。
- Ownership-Integrity Self-Check / Lock Lifecycle ContractのTOCTOU残存window（Accepted・Low確率・未解消、25章R2）：OS標準APIにアトミックなcompare-and-deleteプリミティブが存在しないため、本Releaseのスコープでは解消しない。
- `RetryRuntimeLock`等の`Retry`命名を冠する汎用コンポーネントのリネーム（Open Issue、25章R4）：本Releaseでは実施しない。
- `docs/CHANGELOG.md` `[KI-33]`（untrackedファイルに起因するFormal Regression 5ファイルのFAIL）：**本Releaseのcommit自体が解消条件**であり、Implementation Phase自身のFuture Extensionではないが、commit実施時に必ず再確認すること。post-commit再確認の結果、4ファイルは自然解消を確認したが、`test_e2e_v6_27_0`のみ別種のhistorical guard mapping漏れ（`ZERODIFF-1[scripts]`）が判明し、`[KI-33]`のpost-commit追記が記録する狭い補正で解消済み。

詳細は`docs/design/scheduler_driver_duplicate_dispatch_safety_foundation.md`（Architecture Design、Codex Round 1〜5の記録を含む）を参照。
...
## [v6.32.0] - 2026-09-12 ★ Side-Effect Fail-Closed & Human Review Safety

> `docs/MVP_COMPLETION_ROADMAP.md`が定める6.32スコープに対応し、partial success・
> 既に外部副作用（WordPress下書き作成・media upload等）が発生済みのrunを、
> write-aheadのfail-closed契約とdurableなHuman Review terminal dispositionにより
> 安全に扱う契約を確立した。
>
> Architecture Design（`docs/design/side_effect_fail_closed_human_review_safety_foundation.md`）
> 確定後、Final Release Review中にCodexが指摘した「`RetryExecutor.execute()`・
> `RetryLineageManager._reconcile_all_locked()`のいずれも
> `SideEffectSafetyClassifier.classify()` / `resolve_final_disposition()`を呼んでおらず、
> 6.31時代のstep-onlyな判定のみでdispositionを確定している」というBlocking findingを受け、
> Architecture Amendment（`docs/design/side_effect_fail_closed_human_review_safety_amendment_protected_operation_manifest.md`、Protected Operation Manifest）を新設した。
> Codex独立adversarial review Round 1〜10を実施し、Round 9でSIMPLIFICATION PIVOT
> （rebind機構・lease機構を撤廃し、`member_run_id`をmanifest永続化キーへ組み込む
> IMMUTABLE EXECUTION/MEMBER GENERATION設計へ移行）を経て、Round 10で
> **APPROVED**（Blocking 0／Major 0／Minor 1／Suggestions 2）に収束した。

### Added

- `src/protected_operation_manifest/` 新規パッケージ：`ProtectedOperationManifestStore`
  （`root_run_id` + `attempt_ordinal` + `member_run_id`のimmutable複合キーによる
  durable manifest。create専用`create_for_attempt()`＋追記専用`register()`＋
  read-only`list_for_attempt()`のみを公開）・`ManifestReaderFacade` /
  `ManifestRegistrarFacade`（role-scoped concrete runtime facade）・
  `ProtectedOperationManifestStoreLock`。
- `src/side_effect_safety/` 新規パッケージ：`SideEffectExecutionMode`
  （`RetryLineageProtectedExecutionContext` / `LegacyDirectExecutionContext`の
  discriminated union、Trusted Composition Boundary Invariant）・
  `MediaUploadSafetyCoordinator`（owner_token不要のdurable state machine）・
  `CommitAwareLock`。
- `src/wordpress_draft_state/` 新規パッケージ：`WordPressDraftStateManager` /
  `WordPressDraftStateStore`（WORDPRESS_DRAFT_CREATIONのIO_ARMEDフェーズを持たない
  単一フェーズdurable state）。
- `src/side_effect_safety_classifier.py`：`SideEffectSafetyClassifier`
  （`MediaUploadSafetyCoordinator` / `WordPressDraftStateStore`双方を読み取り、
  `SideEffectSafetyCategory`へ写像する統合層）。
- `src/retry_composition/retry_after_human_review.py`：HRR authorized retryの
  queue/scheduler非経由directディスパッチ。
- `owner_token` durable claim authority（`ClaimResult.owner_token`・
  `mark_execution_started()` / `release_claim()`の4段階authority check、
  stale caller保護）。
- v6.32系列E2E（`tests/test_e2e_v6_32_0_*.py`〜`test_e2e_v6_32_37_*.py`、計39ファイル）。

### Changed

- `src/retry_engine/retry_executor.py` / `src/retry_lineage/retry_lineage_manager.py`：
  `SideEffectSafetyClassifier.classify()` → `resolve_final_disposition()`の実チェーンを
  sync（`execute()`）・restart（`_reconcile_all_locked()`）双方へ接続。manifest entryから
  `member_run_id`を厳密解決（sync=`engine_result.run_id`、restart=`record.latest_run_id`）。
  Final Independent Codex Review Blocking#1対応：protected lineage
  （`side_effect_contract_version is not None`）でmanifest/classifierが欠落している
  場合、決してstep-only fallback（`decide_disposition()` / `disposition_from_categories()`）
  へ降格せず、fail-closedで`HUMAN_REVIEW_REQUIRED`へ確定するよう修正（legacy lineageの
  fallbackは維持）。
- `src/outputs/wordpress_output.py`（呼び出し箇所A）・`main.py`
  `_apply_featured_media_step()`（呼び出し箇所B）・`src/ai/ai_publish_service.py`
  （呼び出し箇所C）：`side_effect_execution_context`必須化・manifest registration
  （既存write-ahead呼び出しより先に実行、durable ACK確認）を追加。Final Independent
  Codex Review Blocking#2対応：protected contextでの`manifest_registrar`省略による
  registration bypassを禁止し、`SideEffectExecutionModeContractError`
  （`ExecutionModeFailureReasonCode.PROTECTED_MANIFEST_REGISTRAR_REQUIRED`、既存例外
  クラスへの理由コード追加のみ）で即座に拒否するよう修正（legacy contextの省略は
  引き続き許容）。
- `src/side_effect_safety/side_effect_execution_mode.py`：
  `ExecutionModeFailureReasonCode`へ`PROTECTED_MANIFEST_REGISTRAR_REQUIRED`を追加。
- `src/retry_composition/retry_composition_root.py`：`ProtectedOperationManifestStore`・
  `SideEffectSafetyClassifier`のインスタンス化、`RetryLineageManager`へフルアクセス
  store、`RetryExecutor`へ`ManifestReaderFacade`、呼び出し箇所A/B/Cへ
  `ManifestRegistrarFacade`のみを配線。
- MEDIA Invariant #15：`RetryCompositionRoot`が`main.py`とは独立した第2の
  `MediaUploadSafetyCoordinator`インスタンスをcross-process read-only classification
  専用に構築することを、既存static oracleの実装齟齬として明確化（Invariant #15
  自体のsemantics・mutation authorityの閉包性は無変更、詳細はAmendment §33）。
- `src/ai/agent_context.py` / `agent_executor.py` / `agent_manager.py` /
  `news_agent.py` / `publish_trigger_agent.py` / `workflow_context.py` /
  `workflow_runner.py` / `workflow_step_executor.py` / `workflow_trigger_agent.py`、
  `src/pipeline/news_pipeline_runner.py` / `publish_pipeline_runner.py` /
  `workflow_pipeline_runner.py`、`scripts/run_news_agent.py` 等7スクリプト：
  Explicit Side-Effect Execution Mode専用channelの新設・伝播。
- `tests/zero_diff_guard_registry.py`：`RELEASE_ORDER`へ`"v6.32.0"`をappend-onlyで
  追記。`_TEST_CHANGE_CONTRIBUTIONS` / `_SOURCE_CHANGE_CONTRIBUTIONS`へ本Release分の
  寄与を登録（v6.32系列39ファイル自身・上記sourceの変更対象path・GR-7準拠のtest
  追加登録を含む）。既存recordはいずれも無改修。

### Architecture Reviewの経緯（Codex独立review、Amendment Round 1〜10 + Final Independent Codex Review）

Architecture Amendment（Protected Operation Manifest）はCodex Round 1〜9で
`NEEDS_REVISION`を繰り返し（Round 9でSIMPLIFICATION PIVOT）、Round 10で
**APPROVED**（Blocking 0／Major 0／Minor 1／Suggestions 2）に収束した。実装完了後の
**Final Independent Codex Review**は1回目`CHANGES REQUIRED`（**Blocking 2**：
protected lineageのstep-only fallback降格リスク／呼び出し箇所A/B/Cのmanifest
registration bypassリスク）を検出し、HUMAN GATE承認のもと限定修正（新しい
architecture判断ではなく既存fail-closed semanticsのproduction反映）を実施、
2回目**APPROVED WITH SUGGESTIONS**（Blocking 0／Major 0／Minor 0／
**Suggestion 1 non-blocking**）に収束した。

Suggestion（Release safety blockerではない、future improvementとして記録・本
Releaseでは未対応）：`RetryExecutor.execute()`の`release_claim()`戻り値未確認による
diagnostic reason（"claim released back to READY_ELIGIBLE"）が実durable stateと
ずれうる既存（6.31由来）のgap。durable stateはfail-closedのまま維持され、既存
`reconcile_all()`のCLAIMED orphan回収で安全に収束することを確認済み。

### §18 Implementation Matrix・Test Review・Regressionの実績

Amendment §18が定めるPLANNED IMPLEMENTATION TEST MATRIX（全25項目）は、production
wiring（`RetryExecutor.execute()`実チェーン・`RetryLineageManager.reconcile_all()`
実チェーン）を通した直接証拠に基づき**25/25 PASS（PARTIAL 0・MISSING 0）**を確認した
（test#22・test#25はhard-crash方式：別プロセス＋`os._exit()`で実証）。

v6.32系列Full Suite（`tests/test_e2e_v6_32_*.py`、実ファイル39件）は
**1270/1270 PASS、FAIL 0、SKIP 0、全ファイルexit code 0**（Blocking remediation後の
再実行分を含む）。正式Formal Regression（正式Inventory34ファイル：v1.11.0＋v5.9.0＋
v6.0.0〜v6.31.0）は**5671/5671 PASS、FAIL 0、SKIP 0、全ファイルexit code 0**で完了した。

詳細は`docs/design/side_effect_fail_closed_human_review_safety_foundation.md`
（Architecture Design）・`docs/design/side_effect_fail_closed_human_review_safety_amendment_protected_operation_manifest.md`
（Architecture Amendment、Codex Round 1〜10・§18 Implementation Verification Record
（§34）・Final Independent Codex Review & Blocking Remediation（§35）を含む全35章）を
参照。

### Future Extension

`RetryObservabilityPipeline`（v6.29.0）のRetry Runtime実配線（6.33、Retry
Observability Runtime Integration）。generic non-HRR automatic retry redispatch等、
本Amendmentが明示的にOut of Scopeとした事項は本Releaseで解決済みではない。
...
> Review 1：Changes Required（Major 4／Minor 2／Suggestion 2、最小改訂で反映済み）→
> Architecture Review 2：**Approved with Suggestions**（Blocking 0／Major 0／Minor 0／
> Suggestion 2）／Test Review／Implementation前Gate確認：**PASS**（baseline
> v6.19〜v6.24、1628/1628 PASS実測）／Production Implementation：**Completed**
> （Production 7ファイル）／New E2E：**Completed**（`test_e2e_v6_25_0_*.py`新規、
> 既存6ファイル更新）／Code Review 1：**Changes Required**（Blocking 0／Major 3／
> Minor 2／Suggestion 5）→ 修正 → Code Review 2：**Approved with Suggestions**
> （Blocking 0／Major 0／Minor 1／Suggestion 5、新規Minorはdesign doc側で
> 解消済み）。**限定回帰（v6.19〜v6.25の関連7ファイルのみ対象、履歴として維持）：
> 1792/1792 PASS。** Documentation Integration：Completed。**Formal Regression：
...
- `tests/test_e2e_v6_25_0_image_generation_fallback_observability_foundation.py`
  新規作成（Scenario prefix：OBSERVATION-／REASONNORM-／CONTINUE-OBS-／
  PROPAGATE-OBS-／LOGFIELD-／NULLLOG-／DEDUP-／SECRET-FREE-／SCHEMA-COMPAT-／
  NOIMPACT-／NOLEAK-／RUNTIMEPATH-／RUNTIME-E2E-／SEC-GUARD-、**128アサーション、
  128/128 PASS**）。`RUNTIME-E2E-`は`main.main()`をRSS収集・重要度判定・記事生成・
  画像Runtime・WordPress/Markdown出力・LogManagerのすべてをmonkeypatchしたうえで
  実際に実行するbehavioral E2Eであり、CONTINUE-WP-SUCCESS／CONTINUE-WP-FAILURE／
  APPLIED／DISABLED／PROPAGATE／NULLLOGの6経路をcounter・Markdown保存回数・
  loop継続・observation内容・記事間NOLEAKを含めて検証する
- `src/image_generation_fallback_policy/image_generation_fallback_policy.py`へ
  `extract_safe_reason(error: Exception) -> str | None`を新規追加。
  ...
  allow-list**（各例外型は自分自身のreason型としか組まない）。未知の例外型・
  誤った型の組合せ・reason属性欠落・想定外のreason型はすべて`None`へ落ちる。
  未知の例外型に対しては`.reason`へ一切アクセスしない防御的順序（`isinstance`
  確認が先）を採用し、`str(error)`／`repr(error)`／`type(error).__name__`は
  いずれも参照しない
- `src/article_featured_media_runtime/article_featured_media_runtime.py`へ
  ...
  `reason`の3 field）と、module-private `_build_observation()`を新規追加。
  `ArticleFeaturedMediaRuntime.classify_propagated_failure(error)`を新規追加
  （PROPAGATE後、呼び出し側がobservability目的で失敗を分類する読み取り専用API）
- `src/logger/log_entry.py`の`ArticleLogEntry`へ`featured_media_category`／
  `featured_media_action`／`featured_media_reason`（いずれも`str = ""`）を
  末尾追加（既存18 field・順序・デフォルト値は無変更）
- `docs/design/image_generation_fallback_observability_foundation.md`
  新規作成（Architecture Design〜Code Review 2までの全経緯を記録）

### Changed

- `src/image_generation_fallback_policy/__init__.py`：`__all__`を
  **既存4→5 symbol**へ拡張（`extract_safe_reason`追加。既存4 symbolの意味・
  signatureは無変更）
- `src/article_featured_media_runtime/__init__.py`：`__all__`を
  **既存3→4 symbol**へ拡張（`FeaturedMediaFailureObservation`追加）
- `src/article_featured_media_runtime/article_featured_media_runtime.py`：
  `ArticleFeaturedMediaRuntimeResult`へ`observation: FeaturedMediaFailureObservation
  | None = None`を末尾追加（既存`article`／`status`／`category`の3 fieldは無変更）。
  `apply()`のCONTINUE分岐で、既存の`decide_image_generation_fallback()`呼び出し
  （1回のみ、再評価しない）から`_build_observation()`経由でobservationを構築し
  `Result`へ格納する
- `src/logger/log_manager.py`：`LogManager.log_article()`**および
  `NullLogManager.log_article()`の双方**へ`featured_media_category`／
  `featured_media_action`／`featured_media_reason`（いずれも`str = ""`）を追加。
  両クラスは常に同一のキーワード引数集合を受け付ける対称性を維持する
- `main.py`：
  - `_apply_featured_media_step()`の戻り値を`ArticleData`から
    `ArticleFeaturedMediaRuntimeResult`そのものへ変更
  - `_handle_featured_media_failure()`へ`observation`引数を追加
    （デフォルト`None`。`None`の場合は3フィールドとも`""`）
  - 記事ループ内の`except Exception:`を`except Exception as exc:`へ変更。
    `exc`の唯一の使用は`featured_media_runtime.classify_propagated_failure(exc)`
    の引数位置のみ（`str(exc)`／`repr(exc)`／`type(exc).__name__`・raw exceptionの
    ログ渡し・属性改変はいずれも行わない）
  - 各記事の反復ごとに`featured_media_observation`を毎回`None`へ再初期化
    （記事間でobservationが漏れないことを保証する唯一の箇所）
  - 通常系（成功／失敗／スキップ）の`log_manager.log_article()`呼び出しへ
    `featured_media_category`／`featured_media_action`／`featured_media_reason`を追加
- 既存E2E 7ファイルの更新（GR-9：保護対象パスへ触れるReleaseは既存の全baseline
  固定guardのallow-listを更新する）：
  - `test_e2e_v6_19_0_image_generation_fallback_policy_foundation.py`：
    `API-ALL-EXACT`を5 symbolへ、`DEP-ISINSTANCE-TARGETS-LIMITED`の許可型集合へ
    `WordPressMediaUploadErrorReason`を追加
  - `test_e2e_v6_20_0_article_featured_media_runtime_foundation.py`：
    `API-ALL-EXACT`を4 symbolへ、`RESULT-FIELDS`へ`observation`を追加、
    `COMPAT-V619`を5 symbolへ更新
  - `test_e2e_v6_21_0_article_featured_media_runtime_wiring.py`：
    `LOOP-HANDLER-NO-BINDING`（`_handler.name is None`という実装詳細の検査）を、
    SEC-2／SEC-3が本来禁じる操作（`str(exc)`／`repr(exc)`／class名出力／
    raw exceptionのログ渡し／属性改変／許可外使用）を直接検査する6つの
    positive allow-list guardへ精緻化。`run_apply_step_capturing_stdout`等の
    戻り値取り出しを`.article`／`.observation`経由へ更新。`MDOK-OBS-`セクションを
    新設し、observation非None時の`log_article()`転記を検証
  - `test_e2e_v6_22_0_wordpress_media_upload_failure_reason_classification_foundation.py`：
    `COMPAT-V619`／`COMPAT-V620`の期待値更新、GR-9 allow-list更新
  - `test_e2e_v6_23_0_openai_image_generation_api_rejection_reason_classification_foundation.py`：
    `COMPAT-POLICY-ALL`の期待値更新、GR-9 allow-list更新（GR-6のequality検査対象にも含める）
  - `test_e2e_v6_24_0_openai_image_generation_unknown_and_invalid_response_reason_refinement_foundation.py`：
    `COMPATAPI-POLICY-ALL`の期待値更新。`POLICYFILE-DIFF-EMPTY`（whole-file無変更
    → 2ファイルのみ許容するallow-list化）・`POLICYFILE-AST-EQUAL`（whole-file AST等価
    → `decide_image_generation_fallback()`関数単位のAST等価へ縮小）。失われた
    保護範囲（`_ACTION_BY_CATEGORY`／`_CONTINUABLE_REASONS`／Enum構成／分類ロジック
    全体）は、v6.25新規`NOIMPACT-*`・v6.23の`COMPAT-*`・v6.19の全網羅テストで
    代替的にカバーされていることを確認した

### Contract概要

- `image_generation_fallback_policy`の分類ロジック本体
  （`decide_image_generation_fallback()`・`_ACTION_BY_CATEGORY`・
  `_CONTINUABLE_REASONS`・`_REJECTED_REASONS`）は無改修。**category/action
  mapping・CONTINUE対象4値はいずれも不変**
- `extract_safe_reason()`はsecret-free契約を満たす：`str(error)`／`repr(error)`／
  `type(error).__name__`はいずれも参照しない。既知2型の組合せに一致する場合のみ
  reasonの`.value`を返し、それ以外（未知型・誤組合せ・属性欠落・未知reason型）は
  すべて`None`。**正式に定義済みの`UNKNOWN`（例：`"unknown"`）は既知値として
  記録され、`None`（reasonが取得できなかった状態）とは明確に区別される**
- PROPAGATE時の`bare raise`・例外の同一性（`id()`不変）・`message`・
  `__cause__`／`__context__`のchainingはいずれも不変。`classify_propagated_failure()`
  は例外を読み取るのみで変更・再送出・wrapを一切行わない
- `main.py`は`article_featured_media_runtime`以外の画像系packageを
  一切importしない（`GUARD-NO-OTHER-IMAGE-PACKAGE`維持）
- 全記事のJSON Linesキー集合は変化する（fallback未発生記事も含め新3キーが
  常時追加される。値は`""`）。この後方互換性は**確認済みRepository内consumer
  （`analytics_manager.py`等）に対してのみ**保証され、外部・未知のstrict schema
  consumerには非互換となり得る

### Runtime不変条件

> v6.24.0のRuntime Action Zero Diff（Z-1〜Z-8）は**過去Releaseの成立事実として
> 変更しない**。v6.25.0では新しいZero Diff名称を作らず、以下を個別の不変条件
> （INV-1〜INV-7）として定義する。
...
これはv6.24というRelease時点でZ-3が成立していたという記録を書き換えるもの
ではなく、v6.25という別Releaseにおいて新たに不成立となるという事実である。

### Tested

- **限定回帰（v6.19〜v6.25の関連7ファイルのみ対象）：合計1792/1792 PASS**
  - v6.19.0：**273/273**（既存272 → 273、**+1**。`API-ALL-EXACT`の`__all__`が
    4→5 symbolへ拡張されたことに伴い、`API-EXPORT-EXISTS[{name}]`ループの
    反復回数が1件増加した構造的追随）
  - v6.20.0：**200/200**（既存198 → 200、**+2**。`article_featured_media_runtime.__all__`
    3→4 symbol拡張に伴う`API-EXPORT-EXISTS`ループの構造的追随+1、および新規
    `RESULT-OBSERVATION-DEFAULT-NONE`+1）
  - v6.21.0：**170/170**（既存147 → 170、**+23**。`MDOK-OBS-`新設・
    `GATEOFF`/`APPLIED`/`CONT`各セクションの`.observation`検証追加・
    `LOOP-HANDLER`精緻化6guard分の純増）
  - v6.22.0：324/324（既存324から件数不変。`COMPAT-V619`／`COMPAT-V620`の
    内容更新のみ）
  - v6.23.0：**345/345**（既存341 → 345、**+4**。`COMPAT-POLICY-ALL`は内容更新のみ。
    `_allowed_source_changes`へ`article_featured_media_runtime`／`logger`の2エントリを
    追加したことに伴う、GR-6 equality検査（`NOIMPACT-SCOPE-COVERAGE`／
    `NOIMPACT-SCOPE-EXACT`）ループの構造的追随）
  - v6.24.0：**352/352**（既存346 → 352、**+6**。`COMPATAPI-POLICY-ALL`内容更新・
    `POLICYFILE-*`guard縮小に加え、同様の`_allowed_source_changes`エントリ追加に
    伴うequality検査ループの構造的追随）
  - 新規v6.25.0：**128/128**
- **これは正式Inventory全ファイルを対象とするFormal Regressionではない
  （履歴として維持）。**
- **Formal Regression（確定）：正式Inventory28ファイル（v1.11.0＋v5.9.0＋
  v6.0.0〜v6.25.0）、合計4582/4582 PASS、FAIL 0／SKIP 0、全ファイルexit code 0**
  - 新規v6.25.0：128/128 PASS
  - 旧27ファイル分：4454/4454 PASS（4582−128）。Release 6.24.0時点の文書記録値
    4418との差分+36は、上記限定回帰の構造的増分（v6.19 +1／v6.20 +2／
    v6.21 +23／v6.22 +0／v6.23 +4／v6.24 +6＝36）と完全に一致
  - INV-1〜INV-7・R-5（`test_e2e_v6_20_0_*.py`のSEC節が`observation`field導入後も
    PASSすること）はいずれも確認済み
  - 実行は`.\venv\Scripts\python.exe`のみ使用。実行後の`git status`は実行前と
    完全に同一で、テスト実行による想定外の差分はなかった

### Deferred

- **s-1〜s-5（v6.24.0由来、継続）**：signature検証の`inspect.signature`ベース化・
  `NOVALPARSE-SCOPE-FUNCTION-ONLY`陽性対照の意図明確化・
  `NOIMPACT-POSITIVE-EMPTY-ALLOWLIST`の論理的重複整理・
  `openai_image_generator.py`長行の折り返し・設計書内相対表現の字義的不整合。
  いずれも本Releaseでは対応せず、次にtestsへ触れるReleaseで併合検討する
- **s-7（v6.24.0由来、継続）**：I-VAL-1陰性対照W-1／W-2の重複（検出力低下なし）。
  s-3と合わせて、テストスイート自体に触れる次のReleaseで整理する候補とする
- **DI-9（Gate値 strict validation）**：本Release対象外
- **s-3／s-7の本Release独自の扱い**：本Releaseの新規Suggestion（S-1〜S-5、
  Code Review 1／2で検出）とは別物であり、混同しないこと。本Release独自の
  S-1〜S-5もDeferredのまま次Releaseへ持ち越す
- **WordPress側CONTINUE対象拡大（DEF-6.22-1）**：本Release対象外。本Releaseが
  観測基盤を確立したことで、将来この判断に必要な運用データが得られる前提が
  整った（本Release自体はCONTINUE対象4値を拡大していない）
- DI-6／DI-7／DI-8：本Release対象外。既存の状態を維持

### Non-Blocking Suggestions（本Release独自、5件）

Code Review 1（Changes Required：Blocking 0／Major 3／Minor 2／Suggestion 5）で
検出したMajor 3件・Minor 2件はいずれも反映済み（Code Review 2：Approved with
Suggestions、Blocking 0／Major 0／Minor 0／Suggestion 5）。Suggestion 5件は
非ブロッキングのままDeferredとする。

- **S-1**：`LOGFIELD-TYPE-STR`が`Field.type`を`str`クラスと比較する検査。
  `log_entry.py`に`from __future__ import annotations`が追加されると
  `"str"`文字列になり破綻しうる
- **S-2**：`DEDUP-LINE-COUNT-2`のログファイル取得が日付ベースのため、日跨ぎ
  実行でFAILしうる（極低確率）
- **S-3**：v6.25新規E2Eの未使用変数（`_date_str_for_glob`）
- **S-4**：`RUNTIMEPATH-NORMAL-LOG-CALL-COUNT`のgenerator再生成パターンが
  読みにくい（O(n²)だが正しい）
- **S-5**：`NULLLOG-SIGNATURE-PARITY`がパラメータ名の集合のみを比較し、
  既定値・kindの差異は検出しない
...
> **本Entryの時点でのRelease状態**：Architecture Design Completed／Architecture Review 1
> Changes Required（Blocking 1件・Major 2件・Minor 5件・Suggestion 3件）／Architecture
> Amendment 1 Completed／Architecture Review 2 Approved with Suggestions（Blocking 0件・
> Major 0件・Minor 2件・Suggestion 1件）／Architecture Amendment 2 Completed／Production
> Implementation Completed／New E2E PASS／Production Code Review Approved with
> Suggestions（Blocking 0件・Major 0件・Minor 1件・Suggestion 3件）／Production
> Implementation Correction Completed（Minor-1解消・Suggestion-1追加反映。
> Suggestion-2・Suggestion-3は非Blockingのまま残存）／Formal Regression Completed／
> Documentation Integration Completed／
> **Release Review：Approved with Suggestions（Blocking 0件・Major 0件・Minor 2件・
> Suggestion 1件：RR-M-1「ROADMAP Entryの`[x]`とRelease未実施表現の一時併存」
> （v6.17.0のRR-M-1と同型の既知パターン）・RR-M-2「設計書冒頭の画像系Foundation件数
> 誤記（10→11）」・RR-S-A「E2E docstring Scenario一覧が`ARTICLETYPE-`を含まず
> 22件のまま」、いずれもDocumentation Integration Finalizeで解消）**。
> **Release：Completed**。

### Added

- 新規独立package`src/article_featured_media_runtime/`：Release 6.9.0〜6.19.0で整備
  された画像系Foundationを、`main.py`が唯一参照するFacadeとして単一責務にまとめる
  （Gate評価の委譲・prompt構築・filename構築・Orchestrator呼び出し・fallback policy
  消費）。v6.19.0 `decide_image_generation_fallback()`の唯一のconsumerであり、
  Deferred Item DI-4（Article Featured Media Runtime Wiring）を2 Releaseへ分割した
  前半（Foundation）に相当する（後半のRuntime Wiringはv6.21.0、次候補・未着手）
  - `src/article_featured_media_runtime/__init__.py`：package rootからPublic API
    3 symbolのみを公開
  - `src/article_featured_media_runtime/article_featured_media_runtime.py`：
    `ArticleFeaturedMediaRuntimeStatus`（Enum、3値）・
    `ArticleFeaturedMediaRuntimeResult`（`@dataclass(frozen=True)`）・
    `ArticleFeaturedMediaRuntime`（`__init__(root)` / `from_env()` / `is_available()` /
    `apply(article)`）
- `tests/test_e2e_v6_20_0_article_featured_media_runtime_foundation.py`
  新規作成（23 Scenario prefix、197アサーション、197/197 PASS。Production Code
  Review Finding Minor-1解消のための`ARTICLETYPE-`、Suggestion-1対応の
  `API-FROM-ENV-GATE-ON-FAILFAST`をProduction Implementation Correctionで追加）
- `docs/design/article_featured_media_runtime_wiring.md`新規作成：Repository
  Survey・Architecture Design・Architecture Review 1〜2・Architecture
  Amendment 1〜2・Production Implementation・Production Code Review・Production
  Implementation Correction・Formal Regression・Documentation Integration・
  Release Reviewの経緯を含む
...
- `ArticleFeaturedMediaRuntimeStatus`（Enum）：`DISABLED` / `APPLIED` /
  `CONTINUED_WITHOUT_FEATURED_MEDIA`の3値
- `ArticleFeaturedMediaRuntimeResult(article, status, category=None)`
  （`@dataclass(frozen=True)`。field順序はarticle/status/category。`category`は
  `CONTINUED_WITHOUT_FEATURED_MEDIA`の場合にのみ非None）
- `ArticleFeaturedMediaRuntime`：
  - `__init__(root)`：Constructor Injection（Duck Typing、isinstance検証なし）
  - `from_env() -> ArticleFeaturedMediaRuntime`（classmethod）：
    `ArticleFeaturedMediaCompositionRoot.from_env()`（v6.18.0）への完全委譲のみ。
    Gate OFFなら無効状態、Gate ONかつcredential不足なら既存factoryの
    `ValueError`を無変換伝播（Fail Fast、v6.18.0の契約をそのまま踏襲）
  - `is_available() -> bool`：`root.is_available()`への完全委譲
  - `apply(article: ArticleData) -> ArticleFeaturedMediaRuntimeResult`

### Contract概要

- 処理順序：`isinstance`検証（`ArticleData`以外は固定message`ValueError`） →
  Gate評価（`is_available()`） → `construct_article_image_prompt()`（v6.17.0） →
  `generate_image_filename()`（v6.16.0） → `orchestrator.apply()`（v6.14.0、
  `try`範囲はこの1呼び出しのみ）
- 失敗時：`except Exception`で捕捉し`decide_image_generation_fallback()`
  （v6.19.0、無改修）へ委譲。`PROPAGATE_ORIGINAL_ERROR`の場合はbare`raise`で
  元例外オブジェクトを無変換・同一性維持のまま再送出（wrap・chaining・
  message加工なし）。`CONTINUE_WITHOUT_FEATURED_MEDIA`の場合は
  `status=CONTINUED_WITHOUT_FEATURED_MEDIA`・`category=decision.category`を
  返し、`article`は引数と同一objectのまま（未改変）で返す
- CONTINUE対象はv6.19.0のallow-list`TIMEOUT` / `CONNECTION` / `RATE_LIMIT` /
  `SERVER_ERROR`の4 reasonのみ（本Releaseでの拡大・変更なし）
- `BaseException`は捕捉しない（`except Exception`に限定）
- DISABLED／APPLIED時は`category=None`。CONTINUE時のみ
  `decision.category`（provider中立5値）を保持する
...
- 既存の画像系Foundation11 package（v6.9.0〜v6.19.0）はいずれも無改修・
  未配線のまま（`main.py`への実接続はv6.21.0 Runtime Wiringの責務、未着手）

### Security／Dependency

- raw exception・例外message・prompt・credential・provider応答本文・
  生成画像bytesのいずれも`ArticleFeaturedMediaRuntimeResult`へ保持しない
- `os` / `logging` / `requests` / `socket`をmodule-levelでimportせず、
  module importのいずれも外部I/O・DNS・socket接続を発生させない
- `openai`のimportを引き起こさない（clean subprocessで決定的に検証）

### Tested

- `tests/test_e2e_v6_20_0_article_featured_media_runtime_foundation.py`：
  23 Scenario prefix（API-／STATUS-／RESULT-／ARTICLETYPE-／DISABLED-／
  APPLIED-／ARGS-／SEQ-／CONT-／PROP-／IDENT-／NOMUT-／TRY-／BASE-／AST-／
  URL-／NOIMPACT-／SEC-／DEP-／IMPORT-／SOCKET-／RUNTIME-／COMPAT-）・
  197アサーション・197/197 PASS（終了コード0、警告0、skip 0、Tracebackなし）
- Formal Regression（累積Regression Inventory方式、既存22ファイル＋新規
  v6.20.0 E2E、計23ファイルを個別実行。`tests/`配下の全E2Eの無差別実行は
  していない）：
  - 既存22ファイル（`docs/CHANGELOG.md` v6.19.0 Testedセクション記載の正式対象、
    v1.11.0・v5.9.0・v6.0.0〜v6.19.0）：3044/3044 PASS（Release 6.19 baseline
    完全維持、新規差分なし）
  - 新規v6.20.0 E2E：197/197 PASS
  - 総合：3241/3241 PASS。全23ファイル終了コード0、FAIL 0、SKIP 0、
    既知差分0、外部API実接続0、Git状態不変
- Architecture Review 1：「Changes Required」（Blocking 1件・Major 2件、
  Minor 5件・Suggestion 3件、いずれもArchitecture Amendment 1で解消）
- Architecture Amendment 1：「Completed」
- Architecture Review 2：「Approved with Suggestions」（Blocking 0件・Major
  0件、Minor 2件・Suggestion 1件、いずれもArchitecture Amendment 2で解消）
- Architecture Amendment 2：「Completed」
- Production Implementation：「Completed」
- Production Code Review：「Approved with Suggestions」（Blocking 0件・Major
  0件、Minor 1件：Minor-1「A-1契約（非`ArticleData`入力時の`ValueError`）が
  未検証」、Suggestion 3件：Suggestion-1「`from_env()`Gate ON時Fail-Fast
  委譲が未検証」・Suggestion-2「`SEC-`の一部が構造的に検証力を持たない」・
  Suggestion-3「monkeypatchベースのテストに陽性対照がない」）
- Production Implementation Correction：「Completed」（Minor-1を
  `ARTICLETYPE-`シナリオ（9アサーション）で解消。Suggestion-1を
  `API-FROM-ENV-GATE-ON-FAILFAST`（1アサーション）で対応。Suggestion-2・
  Suggestion-3は非Blockingのため今回対応せず残存、Production Code変更は
  伴わない。修正後の新規E2Eは197/197 PASS）
- Formal Regression：「Completed」（正式Inventory23ファイル、3241/3241
  PASS、FAIL 0、SKIP 0、終了コード異常0、既知差分0、network使用0、
  credential使用0、Git状態不変）
- Documentation Integration：Completed（`docs/ROADMAP.md` /
  `docs/architecture.md` / 本Entryへ反映済み。件数表記を正式値
  （Scenario prefix 23・新規E2E 197アサーション・Formal Regression
  Inventory 23ファイル・総アサーション3241）へ統一）
- Release Review：「Approved with Suggestions」（Blocking 0件・Major 0件、
  Minor 2件：RR-M-1「ROADMAP `docs/ROADMAP.md`のv6.20.0 Entryが`[x]`（完了マーク）
  でありながら本文が『Release Reviewは未実施であり未完了』と記述しており、
  一時的に矛盾していた」（v6.17.0のRR-M-1「ROADMAP Entryの`[x]`とRelease未実施
  表現の一時併存」と同型の既知パターン）・RR-M-2「設計書
  `docs/design/article_featured_media_runtime_wiring.md`冒頭2箇所（表題直下・
  2.1節冒頭）が『画像系10 Foundation』と誤記していたが、直後のテーブルは
  v6.9.0〜v6.19.0の11行を列挙しており、テーブル直後の文も『これら11 package』
  と正しく11と記載していた（自己矛盾）」、Suggestion 1件：RR-S-A「新規E2E
  `tests/test_e2e_v6_20_0_*.py`冒頭docstringのScenario構成一覧が、Production
  Implementation Correctionで追加した`ARTICLETYPE-`を含まず22件のままだった
  （実体は23 prefix）」。RR-M-1・RR-M-2・RR-S-Aはいずれも本Release Review
  反映工程（Documentation Integration Finalize）で解消。Public API・Facade
  責務・Gate→prompt→filename→orchestrator→fallback順序・PROPAGATE時のbare
  raise・CONTINUE4 reason限定・秘密非保持・consumer-less／Runtime Zero Diff・
  v6.21未着手・DI-10／DI-11 Deferredのいずれにも問題なし。Architectureからの
  逸脱なし）を経て、**Release 6.20として完了した**
...
- Consumer-less Foundation・既存production code無改修・既存test無改修・
  ...
  Diff維持
- Out of Scope：Runtime Wiring（DI-4後半、v6.21.0。次候補・未着手）・
  `main.py`への実接続・記事ループ制御・`WordPressMediaUploadError`のreason
  分類（DI-10、Deferred）・OpenAI `REQUEST_REJECTED`の細分化（DI-11、
  Deferred）・observability／loggingの実装（DI-5）
