"""
Retry Enqueue Trigger（v4.6.0、v4.8.0・v4.9.0で拡張）
...
設計方針:
    ...
    - （v4.9.0）enqueue_pending_failures()は`self._history.has_history()`ではなく
      `self._history.get()`を1回だけ呼び出し、その戻り値（RetryHistoryRecord | None）
      から「履歴の有無」（Guard判定用）と「次のattempt番号」（Queue登録用）の両方を導出する。
      `history_record.attempt_count + 1`（履歴なしの場合は1）を`queue.enqueue()`の
      `retry_attempt`へ明示的に渡すようにし、これまで常に1固定だった値を実際の
      試行回数に連動させる（docs/design/retry_attempt_synchronization_foundation.md）。
      RetryEnqueueGuardの判定基準（履歴の有無の二値）自体は本Releaseでも無変更のため、
      現行Guardが履歴ありを無条件でBLOCKする以上、この分岐に実際に到達するのは
      履歴なし（＝attempt=1）のケースのみであり、本Release単体では観測可能な挙動変化は
      発生しない（Guard精緻化は将来Release）。
