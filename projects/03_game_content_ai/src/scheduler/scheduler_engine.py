...
設計方針:
    ...
    - retry（判定・実行失敗時の再試行）、persistence（判定結果の永続化）、
      Windows Task Scheduler / Linux cron連携は、いずれも将来Releaseの拡張候補
      であり本バージョンでは対象外とする
"""
...
class SchedulerEngine:
    ...
    def __init__(self, clock: ClockProvider | None = None):
