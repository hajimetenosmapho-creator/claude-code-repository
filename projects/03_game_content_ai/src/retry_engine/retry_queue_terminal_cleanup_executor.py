"""
Retry Queue Terminal Cleanup Executor（v4.4.0）

RetryQueueTerminalCleanupResult:   RetryQueueTerminalCleanupDecision 1件に対するCleanup
                                    実行結果を保持する軽量データ
RetryQueueTerminalCleanupExecutor: RetryQueueTerminalCleanupDecision のリストを受け取り、
                                    outcome が CLEANUP の項目についてのみ remove_fn を
                                    呼び出し、Queueから該当項目を除去するコンポーネント

命名の相互参照（混同注意）:
    本ファイルはv4.3.0の RetryQueueCleanupExecutor（retry_queue_cleanup_executor.py、
    SKIPPED由来のNOOP専用）とは別の新規コンポーネントである。名称が似ている
    （RetryQueueCleanupExecutor vs RetryQueueTerminalCleanupExecutor）ため混同しないこと。

設計方針:
    - RetryQueueManager / NullRetryQueueManager 型への依存を一切持たない。remove操作は
      呼び出しごとに remove_fn（Callable[[str], RetryQueueResult]）として受け取る
      （retry_queue_cleanup_executor.pyと同じ方針。既存のRetryQueueManager.remove()を
      再利用し、新しいQueueステータス・Dead Letter・隔離Queueは追加しない）。
    - Stateless。RetryQueueTerminalCleanupDecision と remove_fn を受け取り、結果を
      返すだけのメソッドのみを持つ。内部状態を一切保持しない。
    ...
    - RetryQueueResult.outcome が NOT_FOUND / DISABLED であってもエラーとして
      ...
      retry_queue_cleanup_executor.pyと同じ方針）。
"""
...
from .retry_queue_terminal_cleanup_decider import RetryQueueTerminalCleanupDecision
...
class RetryQueueTerminalCleanupResult:
    """1件のRetryQueueTerminalCleanupDecisionに対するCleanup実行結果を保持する軽量データ。"""
...
class RetryQueueTerminalCleanupExecutor:
    """RetryQueueTerminalCleanupDecisionを対象に、CLEANUPの項目のみremove_fnを呼び出すコンポーネント。"""

    def apply_all(
        self, decisions: list[RetryQueueTerminalCleanupDecision], remove_fn: RemoveFn
    ...
    def apply(
        self, decision: RetryQueueTerminalCleanupDecision, remove_fn: RemoveFn
    ) -> RetryQueueTerminalCleanupResult:
        """1件のRetryQueueTerminalCleanupDecisionについて、除去を試行するかどうかを判定し実行する。"""
        if decision.outcome != RetryQueueCleanupOutcome.CLEANUP:
            return RetryQueueTerminalCleanupResult(
                decision=decision,
                attempted=False,
                queue_result=None,
